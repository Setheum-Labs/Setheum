...
use mock::{
	SEEJointSwap, USSDWBTCPair, USSDEDFPair, USSDJointSwap, EDFWBTCPair, DexModule, ExtBuilder, ListingOrigin, Runtime,
...
fn list_provisioning_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			DexModule::trading_pair_statuses(USSDEDFPair::get()),
...
fn enable_diabled_trading_pair_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::enable_trading_pair(RuntimeOrigin::signed(ALICE), USSD, EDF),
			BadOrigin
		);
		...
		assert_noop!(
			DexModule::enable_trading_pair(RuntimeOrigin::signed(ListingOrigin::get()), EDF, USSD),
...
fn enable_provisioning_without_provision_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::enable_trading_pair(RuntimeOrigin::signed(ListingOrigin::get()), USSD, WBTC),
...
fn end_provisioning_trading_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::end_provisioning(RuntimeOrigin::signed(ListingOrigin::get()), USSD, WBTC),
			Error::<Runtime>::UnqualifiedProvision
		);
		System::set_block_number(10);

		assert_eq!(
			DexModule::trading_pair_statuses(USSDWBTCPair::get()),
			TradingPairStatus::<_, _>::Provisioning(ProvisioningParameters {
				min_contribution: (1_000_000_000_000u128, 1_000_000_000_000u128),
				target_provision: (5_000_000_000_000u128, 2_000_000_000_000u128),
		...
		assert_eq!(
			DexModule::initial_share_exchange_rates(USSDWBTCPair::get()),
			Default::default()
		);
		assert_eq!(DexModule::liquidity_pool(USSDWBTCPair::get()), (0, 0));
		assert_eq!(Tokens::total_issuance(USSDWBTCPair::get().dex_share_currency_id()), 0);
		assert_eq!(
			Tokens::free_balance(USSDWBTCPair::get().dex_share_currency_id(), &DexModule::account_id()),
			0
		);
		...
		assert_eq!(
			DexModule::liquidity_pool(USSDWBTCPair::get()),
...
fn abort_provisioning_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::abort_provisioning(RuntimeOrigin::signed(ALICE), USSD, EDF),
			Error::<Runtime>::MustBeProvisioning
		);
		...
		assert_ok!(DexModule::abort_provisioning(RuntimeOrigin::signed(ALICE), USSD, EDF));
		assert_ok!(DexModule::abort_provisioning(RuntimeOrigin::signed(ALICE), USSD, WBTC));
		assert_eq!(
			DexModule::trading_pair_statuses(USSDEDFPair::get()),
			TradingPairStatus::<_, _>::Provisioning(ProvisioningParameters {
				min_contribution: (1_000_000_000_000u128, 1_000_000_000_000u128),
		...
		assert_eq!(
			DexModule::initial_share_exchange_rates(USSDEDFPair::get()),
			Default::default()
		);
		assert_eq!(
			DexModule::trading_pair_statuses(USSDWBTCPair::get()),
...
fn refund_provision_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::refund_provision(RuntimeOrigin::signed(ALICE), ALICE, USSD, EDF),
			Error::<Runtime>::MustBeDisabled
		);
		...
		assert_eq!(
			DexModule::provisioning_pool(USSDEDFPair::get(), ALICE),
			(1_000_000_000_000_000_000u128, 1_000_000_000_000_000_000u128)
		);
		assert_eq!(
			DexModule::provisioning_pool(USSDEDFPair::get(), BOB),
			(0, 600_000_000_000_000_000u128)
		);
		assert_eq!(
			Tokens::free_balance(USSD, &DexModule::account_id()),
			1_100_000_000_000_000_000u128
		);
		assert_eq!(
			Tokens::free_balance(EDF, &DexModule::account_id()),
		...
		assert_eq!(DexModule::provisioning_pool(USSDEDFPair::get(), ALICE), (0, 0));
		assert_eq!(
			Tokens::free_balance(USSD, &DexModule::account_id()),
			100_000_000_000_000_000u128
		);
		...
		assert_eq!(DexModule::provisioning_pool(USSDEDFPair::get(), BOB), (0, 0));
		assert_eq!(
			Tokens::free_balance(USSD, &DexModule::account_id()),
			100_000_000_000_000_000u128
		);
		assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 0);
		assert_eq!(Tokens::free_balance(USSD, &BOB), 900_000_000_000_000_000u128);
		...
		assert_ok!(DexModule::end_provisioning(RuntimeOrigin::signed(ALICE), USSD, WBTC));
		...
		assert_eq!(
			DexModule::provisioning_pool(USSDWBTCPair::get(), BOB),
			(100_000_000_000_000_000u128, 100_000_000_000_000_000u128)
		);
		assert_noop!(
			DexModule::refund_provision(RuntimeOrigin::signed(BOB), BOB, USSD, WBTC),
...
fn disable_trading_pair_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::disable_trading_pair(RuntimeOrigin::signed(ALICE), USSD, EDF),
			BadOrigin
		);
		...
		assert_noop!(
			DexModule::disable_trading_pair(RuntimeOrigin::signed(ListingOrigin::get()), USSD, EDF),
		...
		assert_noop!(
			DexModule::disable_trading_pair(RuntimeOrigin::signed(ListingOrigin::get()), USSD, WBTC),
			Error::<Runtime>::MustBeEnabled
		);
	});
}
...
fn add_provision_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Tokens::free_balance(USSD, &ALICE), 1_000_000_000_000_000_000u128);
		assert_eq!(Tokens::free_balance(EDF, &ALICE), 1_000_000_000_000_000_000u128);
		assert_eq!(Tokens::free_balance(USSD, &DexModule::account_id()), 0);
		assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 0);
		let alice_ref_count_0 = System::consumers(&ALICE);
...
fn claim_dex_share_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::claim_dex_share(RuntimeOrigin::signed(ALICE), ALICE, USSD, EDF),
			Error::<Runtime>::StillProvisioning
		);
		...
		assert_ok!(DexModule::claim_dex_share(RuntimeOrigin::signed(BOB), BOB, USSD, EDF));
...
fn get_liquidity_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DexModule::liquidity_pool(USSDEDFPair::get()), (1000, 20));
		assert_eq!(DexModule::get_liquidity(USSD, EDF), (1000, 20));
		assert_eq!(DexModule::get_liquidity(EDF, USSD), (20, 1000));
	});
}
...
fn get_target_amounts_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::get_target_amounts(&[EDF], 10000),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_target_amounts(&[EDF, USSD, WBTC, EDF], 10000),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_target_amounts(&[EDF, EDF], 10000),
				Error::<Runtime>::InvalidTradingPath,
			);
			assert_noop!(
				DexModule::get_target_amounts(&[EDF, USSD, EDF], 10000),
				Error::<Runtime>::InvalidTradingPath,
			);
			assert_noop!(
				DexModule::get_target_amounts(&[EDF, USSD, SEE], 10000),
				Error::<Runtime>::MustBeEnabled,
			);
			assert_eq!(
				DexModule::get_target_amounts(&[EDF, USSD], 10000),
				Ok(vec![10000, 24874])
			);
			assert_eq!(
				DexModule::get_target_amounts(&[EDF, USSD, WBTC], 10000),
				Ok(vec![10000, 24874, 1])
			);
			assert_noop!(
				DexModule::get_target_amounts(&[EDF, USSD, WBTC], 100),
				Error::<Runtime>::ZeroTargetAmount,
			);
			assert_noop!(
				DexModule::get_target_amounts(&[EDF, WBTC], 100),
...
fn get_supply_amounts_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			LiquidityPool::<Runtime>::insert(USSDWBTCPair::get(), (100000, 10));
			assert_noop!(
				DexModule::get_supply_amounts(&[EDF], 10000),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&[EDF, USSD, WBTC, EDF], 10000),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&[EDF, EDF], 10000),
				Error::<Runtime>::InvalidTradingPath,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&[EDF, USSD, EDF], 10000),
				Error::<Runtime>::InvalidTradingPath,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&[EDF, USSD, SEE], 10000),
				Error::<Runtime>::MustBeEnabled,
			);
			assert_eq!(
				DexModule::get_supply_amounts(&[EDF, USSD], 24874),
				Ok(vec![10000, 24874])
			);
			assert_eq!(
				DexModule::get_supply_amounts(&[EDF, USSD], 25000),
				Ok(vec![10102, 25000])
			);
			assert_noop!(
				DexModule::get_supply_amounts(&[EDF, USSD, WBTC], 10000),
				Error::<Runtime>::ZeroSupplyAmount,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&[EDF, WBTC], 10000),
				Error::<Runtime>::InsufficientLiquidity,
			);
		});
}
...
fn _swap_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(USSD, EDF), (50000, 10000));
			assert_noop!(
				DexModule::_swap(USSD, EDF, 50000, 5001),
				Error::<Runtime>::InvariantCheckFailed
			);
			assert_ok!(DexModule::_swap(USSD, EDF, 50000, 5000));
			assert_eq!(DexModule::get_liquidity(USSD, EDF), (100000, 5000));
			assert_ok!(DexModule::_swap(EDF, USSD, 100, 800));
			assert_eq!(DexModule::get_liquidity(USSD, EDF), (99200, 5100));
		});
}
...
fn _swap_by_path_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(USSD, EDF), (50000, 10000));
			assert_eq!(DexModule::get_liquidity(USSD, WBTC), (100000, 10));
			assert_ok!(DexModule::_swap_by_path(&[EDF, USSD], &[10000, 25000]));
			assert_eq!(DexModule::get_liquidity(USSD, EDF), (25000, 20000));
			assert_ok!(DexModule::_swap_by_path(&[EDF, USSD, WBTC], &[100000, 20000, 1]));
			assert_eq!(DexModule::get_liquidity(USSD, EDF), (5000, 120000));
			assert_eq!(DexModule::get_liquidity(USSD, WBTC), (120000, 9));
		});
}
...
fn add_liquidity_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::add_liquidity(RuntimeOrigin::signed(ALICE), USSD, EDF, 0, 100_000_000, 0, false),
				Error::<Runtime>::InvalidLiquidityIncrement
			);

			assert_eq!(DexModule::get_liquidity(USSD, EDF), (0, 0));
			assert_eq!(Tokens::free_balance(USSD, &DexModule::account_id()), 0);
			assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 0);
			assert_eq!(
			...
			assert_eq!(
				DexModule::get_liquidity(USSD, EDF),
				(5_000_000_000_000, 1_000_000_000_000)
			);
			assert_eq!(Tokens::free_balance(USSD, &DexModule::account_id()), 5_000_000_000_000);
			assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 1_000_000_000_000);
			assert_eq!(
				Tokens::free_balance(USSDEDFPair::get().dex_share_currency_id(), &ALICE),
				10_000_000_000_000
			);
			...
			assert_noop!(
				DexModule::add_liquidity(RuntimeOrigin::signed(BOB), USSD, EDF, 4, 1, 0, true,),
				Error::<Runtime>::InvalidLiquidityIncrement,
			);
			...
			assert_eq!(Tokens::free_balance(USSD, &DexModule::account_id()), 45_000_000_000_000);
			assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 9_000_000_000_000);
			assert_eq!(
				Tokens::free_balance(USSDEDFPair::get().dex_share_currency_id(), &BOB),
				0
			);
...
fn remove_liquidity_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Tokens::free_balance(USSD, &DexModule::account_id()), 1_000_000_000_000);
			assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 200_000_000_000);
			assert_eq!(
				Tokens::free_balance(USSDEDFPair::get().dex_share_currency_id(), &ALICE),
				2_000_000_000_000
			);
...
fn do_swap_with_exact_supply_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				DexModule::get_liquidity(USSD, WBTC),
			...
			assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 100_000_000_000_000);
			...
			assert_noop!(
				DexModule::do_swap_with_exact_supply(&BOB, &[EDF, USSD], 100_000_000_000_000, 250_000_000_000_000,),
				Error::<Runtime>::InsufficientTargetAmount
			);
			assert_noop!(
				DexModule::do_swap_with_exact_supply(&BOB, &[EDF, USSD, WBTC, EDF], 100_000_000_000_000, 0),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::do_swap_with_exact_supply(&BOB, &[EDF, USSD, EDF], 100_000_000_000_000, 0),
				Error::<Runtime>::InvalidTradingPath,
			);
			assert_noop!(
				DexModule::do_swap_with_exact_supply(&BOB, &[EDF, SEE], 100_000_000_000_000, 0),
				Error::<Runtime>::MustBeEnabled,
			);
			...
			assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 200_000_000_000_000);
			assert_eq!(Tokens::free_balance(WBTC, &DexModule::account_id()), 10_000_000_000);
			...
			assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 400_000_000_000_000);
			assert_eq!(Tokens::free_balance(WBTC, &DexModule::account_id()), 4_469_336_163);
			assert_eq!(Tokens::free_balance(USSD, &BOB), 1_000_248_743_718_592_964);
...
fn do_swap_with_exact_target_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::do_swap_with_exact_target(&BOB, &[EDF, USSD], 250_000_000_000_000, 100_000_000_000_000,),
			...
			assert_noop!(
				DexModule::do_swap_with_exact_target(&BOB, &[EDF, USSD, EDF], 250_000_000_000_000, 200_000_000_000_000,),
				Error::<Runtime>::InvalidTradingPath,
			);
			assert_noop!(
				DexModule::do_swap_with_exact_target(&BOB, &[EDF, SEE], 250_000_000_000_000, 200_000_000_000_000),
				Error::<Runtime>::MustBeEnabled,
			);
			...
			assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 201_010_101_010_102);
			assert_eq!(Tokens::free_balance(WBTC, &DexModule::account_id()), 10_000_000_000);
			...
			assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 338_664_681_397_095);
			assert_eq!(Tokens::free_balance(WBTC, &DexModule::account_id()), 5_000_000_000);
			assert_eq!(Tokens::free_balance(USSD, &BOB), 1_000_250_000_000_000_000);
...
fn initialize_added_liquidity_pools_genesis_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(USSD, EDF), (1000000, 2000000));
			assert_eq!(Tokens::free_balance(USSD, &DexModule::account_id()), 2000000);
			assert_eq!(Tokens::free_balance(EDF, &DexModule::account_id()), 3000000);
			assert_eq!(
				Tokens::free_balance(USSDEDFPair::get().dex_share_currency_id(), &ALICE),
				2000000
			);
...
fn get_swap_amount_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			LiquidityPool::<Runtime>::insert(USSDEDFPair::get(), (50000, 10000));
			assert_eq!(
				DexModule::get_swap_amount(&[EDF, USSD], SwapLimit::ExactSupply(10000, 0)),
				Some((10000, 24874))
			);
			assert_eq!(
				DexModule::get_swap_amount(&[EDF, USSD], SwapLimit::ExactSupply(10000, 24875)),
				None
			);
			assert_eq!(
				DexModule::get_swap_amount(&[EDF, USSD], SwapLimit::ExactTarget(Balance::max_value(), 24874)),
				Some((10000, 24874))
			);
			assert_eq!(
				DexModule::get_swap_amount(&[EDF, USSD], SwapLimit::ExactTarget(9999, 24874)),
				None
			);
		});
}
...
fn get_best_price_swap_path_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactSupply(10, 0), vec![]),
				Some((vec![EDF, USSD], 10, 29))
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactSupply(10, 30), vec![]),
				None
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactSupply(0, 0), vec![]),
				None
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactSupply(10, 0), vec![vec![SEE]]),
				Some((vec![EDF, USSD], 10, 29))
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactSupply(10, 0), vec![vec![EDF]]),
				Some((vec![EDF, USSD], 10, 29))
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactSupply(10, 0), vec![vec![USSD]]),
				Some((vec![EDF, USSD], 10, 29))
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactSupply(10, 0), vec![vec![WBTC]]),
				Some((vec![EDF, WBTC, USSD], 10, 44))
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactSupply(10000, 0), vec![vec![WBTC]]),
				Some((vec![EDF, USSD], 10000, 27024))
			);

			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactTarget(20, 30), vec![]),
				Some((vec![EDF, USSD], 11, 30))
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactTarget(10, 30), vec![]),
				None
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactTarget(0, 0), vec![]),
				None
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactTarget(20, 30), vec![vec![SEE]]),
				Some((vec![EDF, USSD], 11, 30))
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactTarget(20, 30), vec![vec![EDF]]),
				Some((vec![EDF, USSD], 11, 30))
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactTarget(20, 30), vec![vec![USSD]]),
				Some((vec![EDF, USSD], 11, 30))
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactTarget(20, 30), vec![vec![WBTC]]),
				Some((vec![EDF, WBTC, USSD], 8, 30))
			);
			assert_eq!(
				DexModule::get_best_price_swap_path(EDF, USSD, SwapLimit::ExactTarget(100000, 20000), vec![vec![WBTC]]),
...
fn get_liquidity_token_address_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DexModule::get_liquidity_token_address(USSD, EDF), None);
		...
		assert_eq!(
			DexModule::get_liquidity_token_address(USSD, EDF),
