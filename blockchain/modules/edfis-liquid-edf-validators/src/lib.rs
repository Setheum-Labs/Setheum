...
//! # Edfis Liquid EDF Validator List Module
//!
//! ## Overview
//!
//! This will require validators to lock some Liquid EDF into insurance fund
//! and if slash happened, EthicalDeFiCouncil can burn those Liquid EDF to compensate
//! Liquid EDF holders.
...
pub const EDFIS_LIQUID_STAKING_VALIDATOR_LIST_ID: LockIdentifier = *b"edf/lqdedfvld";
...
// TODO[src/lib.rs:0]: Do benchmarking test.
impl WeightInfo for () {
	fn bond() -> Weight {
...
pub mod module {
	...
	pub trait Config: frame_system::Config {
		...
		type LiquidEDFCurrency: BasicLockableCurrency<Self::AccountId, Balance = Balance>;
		#[pallet::constant]
	...
	/// The slash guarantee deposits for validators.
	///
	/// Guarantees: double_map AccountId, AccountId => Option<Guarantee>
	#[pallet::storage]
	...
	///
	/// ValidatorBackings: map AccountId => Option<ValidatorBacking>
	#[pallet::storage]
	#[pallet::getter(fn validator_backings)]
	pub type ValidatorBackings<T: Config> =
		StorageMap<_, Blake2_128Concat, T::AccountId, ValidatorBacking, OptionQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		///
		/// - `validator`: the AccountId of a validator to bond to
		...
		pub fn bond(
			...
			let free_balance = T::LiquidEDFCurrency::free_balance(&guarantor);
		...
		///
		/// - `validator`: the AccountId of a validator to unbond from
		...
		/// Rebond tokens to a validator.
		///
		/// - `validator`: The AccountId of a validator to rebond to
		...
		/// Withdraw the unbonded tokens from a validator.
		/// Ensures the validator is not frozen.
		///
		/// - `validator`: The AccountId of a validator to withdraw from
		#[pallet::call_index(3)]
		#[pallet::weight(T::WeightInfo::withdraw_unbonded())]
		pub fn withdraw_unbonded(origin: OriginFor<T>, validator: T::AccountId) -> DispatchResult {
			let guarantor = ensure_signed(origin)?;
			ensure!(
		...
		/// Freezes validators if they are not already frozen.
		/// Ensures the caller can freeze validators.
		///
		/// - `validators`: The AccountIds of the validators to freeze
		#[pallet::call_index(4)]
		#[pallet::weight(T::WeightInfo::freeze(validators.len() as u32))]
		pub fn freeze(origin: OriginFor<T>, validators: Vec<T::AccountId>) -> DispatchResult {
		...
		/// Unfreezes validators if they are frozen.
		/// Ensures the caller can perform a slash.
		///
		/// - `validators`: The AccountIds of the validators to unfreeze
		#[pallet::call_index(5)]
		#[pallet::weight(T::WeightInfo::thaw())]
		pub fn thaw(origin: OriginFor<T>, validators: Vec<T::AccountId>) -> DispatchResult {
		...
		pub fn slash(origin: OriginFor<T>, slashes: Vec<SlashInfo<Balance, T::AccountId>>) -> DispatchResult {
			T::SlashOrigin::ensure_origin(origin)?;
			...
			} in slashes
			{
				...
				for (guarantor, _) in Guarantees::<T>::iter_prefix(&validator) {
					...
					let res = Self::update_guarantee(&guarantor, &validator, |guarantee| -> DispatchResult {
						...
						let gap = T::LiquidEDFCurrency::slash(&guarantor, should_slashing);
								...
								|maybe_validator_backing| -> DispatchResult {
									...
									if tl.is_zero() {
										*maybe_total_locked = None;
										T::LiquidEDFCurrency::remove_lock(EDFIS_LIQUID_STAKING_VALIDATOR_LIST_ID, guarantor)?;
									} else {
										*maybe_total_locked = Some(tl);
										T::LiquidEDFCurrency::set_lock(EDFIS_LIQUID_STAKING_VALIDATOR_LIST_ID, guarantor, tl)?;
									}
...
impl<T: Config> Contains<T::AccountId> for Pallet<T> {
	fn contains(account_id: &T::AccountId) -> bool {
		Self::validator_backings(account_id)
			.unwrap_or_default()
			.total_insurance
