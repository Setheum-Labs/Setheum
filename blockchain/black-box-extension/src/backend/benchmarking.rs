...
//! sometimes we might require that `T` includes some pallet (e.g. `module_vk_storage`). We can put this constraint on
...
/// blanket implementation for all runtimes that contain the `module_vk_storage` pallet.
trait Config: module_vk_storage::Config {}
impl<T: module_vk_storage::Config> Config for T {}
...
macro_rules! generate_benchmarks {
    (
        ...
        paste::paste! {
            ...
            use module_vk_storage::{KeyHasher, VerificationKeys};
            use sp_std::vec;
