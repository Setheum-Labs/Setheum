...
fn simulate_verify<Exc: BackendExecutor>(expected_ret_val: u32) {
    ...
    let result = BlackBoxChainExtension::<SetheumRuntime>::verify::<Exc, _, TestWeight>(env);
...
fn verify__charges_before_reading_arguments() {
    ...
    let result = BlackBoxChainExtension::<SetheumRuntime>::verify::<Panicker, _, TestWeight>(env);
