...
use crate::{
    ...
    extension_ids::{EXTENSION_ID as BLACKBOX_EXTENSION_ID, VERIFY_FUNC_ID},
    status_codes::*,
};
...
pub struct BlackBoxChainExtension<Runtime> {
    _config: PhantomData<Runtime>,
}

impl<Runtime> Default for BlackBoxChainExtension<Runtime> {
    fn default() -> Self {
        Self {
            _config: PhantomData,
        }
...
impl<Runtime: MinimalRuntime> ChainExtension<Runtime> for BlackBoxChainExtension<Runtime>
where
    <Runtime as SystemConfig>::RuntimeOrigin: From<Option<AccountId32>>,
{
    fn call<E: Ext<T = Runtime>>(
        ...
        match (ext_id, func_id) {
            (BLACKBOX_EXTENSION_ID, VERIFY_FUNC_ID) => {
...
impl<Runtime: MinimalRuntime> BlackBoxChainExtension<Runtime>
where
    <Runtime as SystemConfig>::RuntimeOrigin: From<Option<AccountId32>>,
{
