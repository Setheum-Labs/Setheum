...
#[ink::contract(env = black-box_extension::Environment)]
mod test_contract {
    use ink::prelude::vec;
    ...
    mod tests {
        ...
        impl ink::env::test::ChainExtension for MockedVerifyExtension {
            fn ext_id(&self) -> u16 {
                black-box_extension::extension_ids::EXTENSION_ID
            }

            fn call(&mut self, func_id: u16, _: &[u8], _: &mut Vec<u8>) -> u32 {
                assert_eq!(
                    func_id,
                    black-box_extension::extension_ids::VERIFY_FUNC_ID
                );
                black-box_extension::status_codes::VERIFY_SUCCESS
            }
        }
