...
impl From<ink::scale::Error> for SenterpriseError {
    fn from(_: ink::scale::Error) -> Self {
        Self::ScaleError
    }
}

impl ink::env::chain_extension::FromStatusCode for SenterpriseError {
...
/// Senterprise chain extension definition.
...
pub trait SenterpriseExtension {
    type ErrorCode = SenterpriseError;
    ...
    fn verify(
        ...
        proof: Vec<u8>,
        input: Vec<u8>,
    ) -> Result<(), SenterpriseError>;
}

/// Default ink environment with `SenterpriseExtension` included.
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
#[ink::scale_derive(Encode, Decode, TypeInfo)]
...
impl EnvironmentT for Environment {
    ...
    type ChainExtension = SenterpriseExtension;
}
