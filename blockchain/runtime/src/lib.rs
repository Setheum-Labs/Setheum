...
// 	type CurrencyIdMapping = EvmCurrencyIdMapping<Runtime>;
// 	type WeightInfo = weights::edfis_swap_module::WeightInfo<Runtime>;
	...
	fn filter(&self, c: &Call) -> bool {
		match self {
			...
			ProxyType::Swap => {
				matches!(
					c,
					Call::Dex(edfis_swap_module::Call::swap_with_exact_supply(..))
						| Call::Dex(edfis_swap_module::Call::swap_with_exact_target(..))
				)
			}
			ProxyType::Loan => {
				matches!(
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		// Dex: edfis_swap_module::{Pallet, Storage, Call, Event<T>, Config<T>} = 6,
...
impl_runtime_apis! {
	...
	impl frame_benchmarking::Benchmark<Block> for Runtime {
		fn benchmark_metadata(extra: bool) -> (
			...
			// orml_list_benchmark!(list, extra, edfis_swap_module, benchmarking::dex);
		...
		fn dispatch_benchmark(
			...
			// orml_add_benchmark!(params, batches, edfis_swap_module, benchmarking::dex);
