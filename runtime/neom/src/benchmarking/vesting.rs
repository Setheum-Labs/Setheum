...
runtime_benchmarks! {
	...
	vested_transfer {
		...
		set_neom_balance(&from, schedule.total_amount().unwrap() + dollar(NEOM));
	...
	verify {
		assert_eq!(
			<Currencies as MultiCurrency<_>>::total_balance(NEOM, &to),
	...
	claim {
		...
		let from: AccountId = TreasuryPalletId::get().into_account();
		// extra 1 dollar to pay fees
		set_neom_balance(&from, schedule.total_amount().unwrap() * i as u128 + dollar(NEOM));
	...
	verify {
		assert_eq!(
			<Currencies as MultiCurrency<_>>::free_balance(NEOM, &to),
	...
	update_vesting_schedules {
		...
		let to: AccountId = account("to", 0, SEED);
		set_neom_balance(&to, schedule.total_amount().unwrap() * i as u128);
		let to_lookup = lookup_of_account(to.clone());
