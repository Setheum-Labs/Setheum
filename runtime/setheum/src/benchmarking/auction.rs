...
runtime_benchmarks! {
	...
	bid_collateral_auction_as_first_bidder {
		...
		let currency_id = DNAR;
		let collateral_amount = 100 * dollar(currency_id);
		let target_amount = 10_000 * dollar(SETUSD);
		let bid_price = (5_000u128 + d as u128) * dollar(SETUSD);
	...
	bid_collateral_auction {
		...
		let collateral_amount = 100 * dollar(currency_id);
		let target_amount = 10_000 * dollar(SETUSD);
		let previous_bid_price = (5_000u128 + d as u128) * dollar(SETUSD);
		let bid_price = (10_000u128 + d as u128) * dollar(SETUSD);
		let auction_id: AuctionId = 0;
	...
	on_finalize {
		...
		let auction_id: AuctionId = 0;
		set_balance(SETUSD, &bidder, fix_debit_amount * c as u128);
