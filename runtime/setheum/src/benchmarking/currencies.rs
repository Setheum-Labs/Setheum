...
const WBTC: CurrencyId = GetDinarCurrencyId::get();
...
runtime_benchmarks! {
	...
	transfer_non_native_currency {
		let amount: Balance = 1_000 * dollar(WBTC);
		let from: AccountId = whitelisted_caller();
		set_balance(WBTC, &from, amount);

		let to: AccountId = account("to", 0, SEED);
		let to_lookup = lookup_of_account(to.clone());
	}: transfer(RawOrigin::Signed(from), to_lookup, WBTC, amount, false)
	verify {
		assert_eq!(<Currencies as MultiCurrency<_>>::total_balance(WBTC, &to), amount);
	}
	...
	update_balance_non_native_currency {
		let balance: Balance = 2 * dollar(WBTC);
		let amount: Amount = balance.unique_saturated_into();
		let who: AccountId = account("who", 0, SEED);
		let who_lookup = lookup_of_account(who.clone());
	}: update_balance(RawOrigin::Root, who_lookup, WBTC, amount)
	verify {
		assert_eq!(<Currencies as MultiCurrency<_>>::total_balance(WBTC, &who), balance);
	}
	...
	sweep_dust {
		...
		accounts.iter().for_each(|account| {
			orml_tokens::Accounts::<Runtime>::insert(account, WBTC, orml_tokens::AccountData {
				free: 100,
				frozen: 0,
				reserved: 0
			});
		});
		set_balance(WBTC, &treasury, dollar(WBTC));
	}: _(RawOrigin::Root, WBTC, (&accounts[..c as usize]).to_vec())
	verify {
		(&accounts[..c as usize]).iter().for_each(|account| {
			assert_eq!(orml_tokens::Accounts::<Runtime>::contains_key(account, WBTC), false);
		});
		assert_eq!(Tokens::free_balance(WBTC, &treasury), dollar(WBTC) + (100 * c) as Balance);
	}
}
