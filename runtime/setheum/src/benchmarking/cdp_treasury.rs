...
use crate::{dollar, CdpTreasury, Currencies, CurrencyId, Runtime, SETM, SETUSD};
...
runtime_benchmarks! {
	...
	}: _(RawOrigin::Root, currency_id, 1_000 * dollar(currency_id), 1_000 * dollar(SETUSD), true)
	...
	extract_surplus_to_serp {
		CdpTreasury::on_system_surplus(1_000 * dollar(SETUSD))?;
	}: _(RawOrigin::Root, 200 * dollar(SETUSD))
}
