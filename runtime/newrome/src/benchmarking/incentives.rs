...
use crate::{
	...
	Rewards, Runtime, System, TokenSymbol, DNAR, JUSD, DOT,
};
...
runtime_benchmarks! {
	...
	on_initialize {
		...
		for i in 0 .. c {
			...
			Incentives::update_incentive_rewards(RawOrigin::Root.into(), vec![(pool_id.clone(), 100 * dollar(DNAR))])?;
	...
	deposit_dex_share {
		...
		set_balance(BTC_JUSD_LP, &caller, 10_000 * dollar(JUSD));
	}: _(RawOrigin::Signed(caller), BTC_JUSD_LP, 10_000 * dollar(JUSD))
	...
	}: _(RawOrigin::Signed(caller), BTC_JUSD_LP, 8000 * dollar(JUSD))
	...
	update_incentive_rewards {
		...
		for i in 0 .. c {
			let currency_id = currency_ids[i as usize];
			values.push((PoolId::LoansIncentive(currency_id), 100 * dollar(DNAR)));
		}
