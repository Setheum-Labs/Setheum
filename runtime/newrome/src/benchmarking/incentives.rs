...
const SEED: u32 = 0;
const ROME_jUSD_LP: CurrencyId = CurrencyId::DEXShare(TokenSymbol::CHFJ, TokenSymbol::jUSD);
...
runtime_benchmarks! {
	...
	deposit_dex_share {
		...
		set_balance(ROME_jUSD_LP, &caller, 10_000 * dollar(jUSD));
	}: _(RawOrigin::Signed(caller), ROME_jUSD_LP, 10_000 * dollar(jUSD))
	...
	}: _(RawOrigin::Signed(caller), ROME_jUSD_LP, 8000 * dollar(jUSD))

	claim_rewards {
		let caller: AccountId = account("caller", 0, SEED);
		let pool_id = PoolId::Setters(jSETT);
