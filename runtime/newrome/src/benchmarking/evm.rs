...
use crate::{dollar, AccountId, Event, EvmAccounts, Origin, Runtime, System, EVM, DNAR};
...
fn deploy_contract(caller: AccountId) -> Result<H160, DispatchError> {
	...
	System::assert_last_event(Event::setheum_evm(setheum_evm::Event::Created(contract_addr())));
	Ok(contract_addr())
}
...
runtime_benchmarks! {
	...
	transfer_maintainer {
		...
		set_setheum_balance(&alice_account, 1_000_000 * dollar(DNAR));
	...
	enable_contract_development {
		...
		set_setheum_balance(&alice_account, 1_000 * dollar(DNAR));
