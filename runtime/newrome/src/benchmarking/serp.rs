...
runtime_benchmarks! {
	...
	}: _(RawOrigin::Signed(caller), jSETT, to_lookup)
	...
	adjust_setter {
		...
		let standard_value = 100 * dollar(jUSD);
		...
		let reserve_amount = Price::saturating_from_rational(dollar(currency_id), dollar(jUSD)).saturating_mul_int(reserve_value);
