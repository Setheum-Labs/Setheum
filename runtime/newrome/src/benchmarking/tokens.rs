...
runtime_benchmarks! {
	...
	}: _(RawOrigin::Signed(from), to_lookup, SETUSD, amount)
	verify {
		assert_eq!(<Tokens as MultiCurrency<_>>::total_balance(SETUSD, &to), amount);
	}
	...
	}: _(RawOrigin::Signed(from.clone()), to_lookup, SETUSD)
	verify {
		assert_eq!(<Tokens as MultiCurrency<_>>::total_balance(SETUSD, &from), 0);
	}
}
