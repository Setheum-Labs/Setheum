...
fn get_price_of_lp_token_currency_id() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(MockSetheumDex::get_liquidity_pool(JUSD, DNAR), (10000, 200));
		assert_eq!(
			PricesModule::get_price(LP_JUSD_DNAR),
			None
		);
		assert_ok!(Tokens::deposit(LP_JUSD_DNAR, &1, 100));
		assert_eq!(Tokens::total_issuance(LP_JUSD_DNAR), 100);
		assert_eq!(PricesModule::get_price(JUSD), Some(Price::saturating_from_rational(1000000u128, 1)));
		assert_eq!(
			PricesModule::get_price(LP_JUSD_DNAR),
			Some(Price::saturating_from_rational(200000000u128, 1))	// 10000/100 * Price::saturating_from_rational(1000000u128, 1) * 2
		);

		assert_eq!(MockSetheumDex::get_liquidity_pool(JCHF, JUSD), (0, 0));
		assert_eq!(
			PricesModule::get_price(LP_JCHF_JUSD),
			None
		);
	});
}
...
fn get_relative_price_work() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(
			PricesModule::get_relative_price(DNAR, JUSD),
			Some(Price::saturating_from_rational(10000, 1)) /* 1DNAR = 100JUSD, right shift the decimal point (12-10)
			                                                 * places */
		);
		assert_eq!(
			PricesModule::get_relative_price(JCHF, JUSD),
			Some(Price::saturating_from_rational(500000000, 1)) /* 1JCHF = 50000JUSD, right shift the decimal point
			                                                     * (12-8) places */
		);
		assert_eq!(
		...
		assert_eq!(
			PricesModule::get_relative_price(JUSD, JUSD),
			Some(Price::saturating_from_rational(1, 1)) // 1JUSD = 1JUSD, right shift the decimal point (10-10) places
		);
		assert_eq!(PricesModule::get_relative_price(JUSD, DNAR), None);
	});
}
