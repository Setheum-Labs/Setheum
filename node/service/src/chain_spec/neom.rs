...
use crate::chain_spec::{get_account_id_from_seed, get_neom_authority_keys_from_seed, Extensions, TELEMETRY_URL};
...
use neom_runtime::{
		...
		NEOM, MENA, NSETT, JUSD, JEUR, JJPY, JGBP, JAUD, JCAD, JCHF, JSEK, JSAR, RENBTC,
};
use runtime_common::TokenInfo;

pub type ChainSpec = sc_service::GenericChainSpec<neom_runtime::GenesisConfig, Extensions>;

fn neom_session_keys(
	grandpa: GrandpaId,
	babe: BabeId
) -> neom_runtime::SessionKeys {
	neom_runtime::SessionKeys { grandpa, babe }
}

pub fn neom_config() -> Result<ChainSpec, String> {
	ChainSpec::from_json_bytes(&include_bytes!("../../../../resources/neom-dist.json")[..])
}

fn neom_properties() -> Properties {
	let mut properties = Map::new();
	let mut token_symbol: Vec<String> = vec![];
	let mut token_decimals: Vec<u32> = vec![];
	[
		NEOM, MENA, NSETT, JUSD, JEUR, JJPY, JGBP, JAUD, JCAD, JSGD, JSEK, JSAR, RENBTC
	].iter().for_each(|token| {
...
pub fn latest_neom_config() -> Result<ChainSpec, String> {
	...
	Ok(ChainSpec::from_genesis(
		...
		move || {
			...
			let other_allocation_json = &include_bytes!("../../../../resources/neom-allocation-NEOM.json")[..];
			...
			assert_eq!(
				total_allocated,
				2_580_000_000 * dollar(NEOM), // 2.58 billion NEOM
				"total allocation must be equal to 2.58 billion NEOM"
			);

			let vesting_list_json = &include_bytes!("../../../../resources/neom-vesting-NEOM.json")[..];
		...
		vec![
			//TODO: Update!
			//"/dns/testnet-bootnode-1.neom.setheum.xyz/tcp/30333/p2p/12D3KooWAFUNUowRqCV4c5so58Q8iGpypVf3L5ak91WrHf7rPuKz"
			//	.parse()
			//	.unwrap(),
		],
...
pub fn neom_dev_config() -> Result<ChainSpec, String> {
	let wasm_binary = neom_runtime::WASM_BINARY.unwrap_or_default();
	...
	Ok(ChainSpec::from_genesis(
		...
		move || {
			neom_genesis(
				...
				// Initial PoA authorities
				vec![
					get_neom_authority_keys_from_seed("Alice"),
					get_neom_authority_keys_from_seed("Bob"),
					get_neom_authority_keys_from_seed("Charlie"),
				],
				// Sudo account
				get_account_id_from_seed::<sr25519::Public>("Alice"),
				vec![
					(get_account_id_from_seed::<sr25519::Public>("Alice"), 1000 * dollar(NEOM)),
					(get_account_id_from_seed::<sr25519::Public>("Bob"), 1000 * dollar(NEOM)),
					(get_account_id_from_seed::<sr25519::Public>("Charlie"), 1000 * dollar(NEOM)),
					(get_account_id_from_seed::<sr25519::Public>("Alice"), 1000 * dollar(NSETT)),
					(get_account_id_from_seed::<sr25519::Public>("Bob"), 1000 * dollar(NSETT)),
					(get_account_id_from_seed::<sr25519::Public>("Charlie"), 1000 * dollar(NSETT)),
				],
				vec![],
