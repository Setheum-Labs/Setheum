...
pub trait SettCurrency<AccountId> {
	...
	/// A dry-run of `withdraw`. Returns `Ok` if the account is able to make a
...
pub trait ExtendedSettCurrency<AccountId>: SettCurrency<AccountId> {
...
/// restrictions.
pub trait LockableSettCurrency<AccountId>: SettCurrency<AccountId> {
...
pub trait ReservableSettCurrency<AccountId>: SettCurrency<AccountId> {
...
pub trait ExtendedBasicCurrency<AccountId>: BasicCurrency<AccountId> {
...
/// restrictions.
pub trait LockableBasicCurrency<AccountId>: BasicCurrency<AccountId> {
	/// The quantity used to denote time; usually just a `BlockNumber`.
	type Moment;
...
pub trait ReservableBasicCurrency<AccountId>: BasicCurrency<AccountId> {
	/// Same result as `reserve(who, value)` (but without the side-effects)
	/// assuming there are no balance changes in the meantime.
