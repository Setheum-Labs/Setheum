...
//! trading, SettinDEX also participates in transaction fee liquidation.
...
use support::{SettinDexIncentives, SettinDexManager, Price, Ratio};
...
pub mod module {
	...
	pub trait Config: frame_system::Config {
		...
		/// TheSettinDEX's module id, keep all assets in SettinDEX.
		#[pallet::constant]
		...
		/// SettinDex incentives
		type SettinDexIncentives: SettinDexIncentives<Self::AccountId, CurrencyId, Balance>;
	...
	impl<T: Config> Pallet<T> {
		/// Trading with SettinDEX, swap with exact supply amount
		///
		/// - `path`: trading path.
		...
		/// Trading with SettinDEX, swap with exact target amount
		///
		/// - `path`: trading path.
...
impl<T: Config> Pallet<T> {
		...
		LiquidityPool::<T>::try_mutate(trading_pair, |(pool_0, pool_1)| -> DispatchResult {
			...
			*pool_1 = pool_1.saturating_add(pool_1_increment);
			...
			if deposit_increment_share {
				T::SettinDexIncentives::do_deposit_dex_share(who, lp_share_currency_id, share_increment)?;
			}
	...
	fn do_remove_liquidity(
		...
		LiquidityPool::<T>::try_mutate(trading_pair, |(pool_0, pool_1)| -> DispatchResult {
			...
			if by_withdraw {
				T::SettinDexIncentives::do_withdraw_dex_share(who, lp_share_currency_id, remove_share)?;
			}
...
impl<T: Config> SettinDexManager<T::AccountId, CurrencyId, Balance> for Pallet<T> {
