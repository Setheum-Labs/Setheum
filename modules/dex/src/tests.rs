...
use mock::{
	DexModule, Event, ExtBuilder, ListingOrigin, Origin, Runtime, System, Tokens, DNAR, ALICE, JUSD, JUSD_DNAR_PAIR,
...
fn enable_new_trading_pair_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::enable_trading_pair(Origin::signed(ALICE), JUSD, DNAR),
			BadOrigin
		);

		assert_eq!(
			DexModule::trading_pair_statuses(JUSD_DNAR_PAIR),
		...
		assert_noop!(
			DexModule::enable_trading_pair(Origin::signed(ListingOrigin::get()), DNAR, JUSD),
...
fn disable_enabled_trading_pair_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::disable_trading_pair(Origin::signed(ALICE), JUSD, DNAR),
			BadOrigin
		);
		...
		assert_noop!(
			DexModule::disable_trading_pair(Origin::signed(ListingOrigin::get()), JUSD, DNAR),
...
fn disable_provisioning_trading_pair_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Tokens::free_balance(JUSD, &DexModule::account_id()),
			...
			assert_eq!(
				DexModule::provisioning_pool(JUSD_DNAR_PAIR, ALICE),
				(5_000_000_000_000u128, 0)
			);
			assert_eq!(
				DexModule::provisioning_pool(JUSD_DNAR_PAIR, BOB),
			...
			assert_eq!(Tokens::free_balance(JUSD, &DexModule::account_id()), 0);
			assert_eq!(Tokens::free_balance(DNAR, &DexModule::account_id()), 0);
			assert_eq!(DexModule::provisioning_pool(JUSD_DNAR_PAIR, ALICE), (0, 0));
			assert_eq!(DexModule::provisioning_pool(JUSD_DNAR_PAIR, BOB), (0, 0));
			assert_eq!(
				DexModule::trading_pair_statuses(JUSD_DNAR_PAIR),
				TradingPairStatus::<_, _>::NotEnabled
			);
...
fn get_liquidity_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DexModule::liquidity_pool(JUSD_DNAR_PAIR), (1000, 20));
		assert_eq!(DexModule::get_liquidity(JUSD, DNAR), (1000, 20));
		assert_eq!(DexModule::get_liquidity(DNAR, JUSD), (20, 1000));
	});
}
...
fn get_target_amounts_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::get_target_amounts(&vec![DNAR], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![DNAR, JUSD, JCHF, DNAR], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![DNAR, JUSD, DNAR], 10000, None),
				Error::<Runtime>::MustBeEnabled,
			);
			assert_eq!(
				DexModule::get_target_amounts(&vec![DNAR, JUSD], 10000, None),
				Ok(vec![10000, 24874])
			);
			assert_eq!(
				DexModule::get_target_amounts(&vec![DNAR, JUSD], 10000, Ratio::checked_from_rational(50, 100)),
				Ok(vec![10000, 24874])
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![DNAR, JUSD], 10000, Ratio::checked_from_rational(49, 100)),
				Error::<Runtime>::ExceedPriceImpactLimit,
			);
			assert_eq!(
				DexModule::get_target_amounts(&vec![DNAR, JUSD, JCHF], 10000, None),
				Ok(vec![10000, 24874, 1])
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![DNAR, JUSD, JCHF], 100, None),
				Error::<Runtime>::ZeroTargetAmount,
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![DNAR, JCHF], 100, None),
...
fn get_supply_amounts_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			LiquidityPool::<Runtime>::insert(JUSD_JCHF_PAIR, (100000, 10));
			assert_noop!(
				DexModule::get_supply_amounts(&vec![DNAR], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![DNAR, JUSD, JCHF, DNAR], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![DNAR, JUSD, DNAR], 10000, None),
				Error::<Runtime>::MustBeEnabled,
			);
			assert_eq!(
				DexModule::get_supply_amounts(&vec![DNAR, JUSD], 24874, None),
				Ok(vec![10000, 24874])
			);
			assert_eq!(
				DexModule::get_supply_amounts(&vec![DNAR, JUSD], 25000, Ratio::checked_from_rational(50, 100)),
				Ok(vec![10102, 25000])
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![DNAR, JUSD], 25000, Ratio::checked_from_rational(49, 100)),
				Error::<Runtime>::ExceedPriceImpactLimit,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![DNAR, JUSD, JCHF], 10000, None),
				Error::<Runtime>::ZeroSupplyAmount,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![DNAR, JCHF], 10000, None),
				Error::<Runtime>::InsufficientLiquidity,
			);
		});
}
...
fn _swap_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(JUSD, DNAR), (50000, 10000));
			DexModule::_swap(JUSD, DNAR, 1000, 1000);
			assert_eq!(DexModule::get_liquidity(JUSD, DNAR), (51000, 9000));
			DexModule::_swap(DNAR, JUSD, 100, 800);
			assert_eq!(DexModule::get_liquidity(JUSD, DNAR), (50200, 9100));
		});
}
...
fn _swap_by_path_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(JUSD, DNAR), (50000, 10000));
			assert_eq!(DexModule::get_liquidity(JUSD, JCHF), (100000, 10));
			DexModule::_swap_by_path(&vec![DNAR, JUSD], &vec![10000, 25000]);
			assert_eq!(DexModule::get_liquidity(JUSD, DNAR), (25000, 20000));
			DexModule::_swap_by_path(&vec![DNAR, JUSD, JCHF], &vec![4000, 10000, 2]);
			assert_eq!(DexModule::get_liquidity(JUSD, DNAR), (15000, 24000));
			assert_eq!(DexModule::get_liquidity(JUSD, JCHF), (110000, 8));
		});
}
...
fn add_liquidity_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::add_liquidity(Origin::signed(ALICE), DNAR, JUSD, 100_000_000, 100_000_000, false),
				Error::<Runtime>::NotEnabledTradingPair
			);
			assert_noop!(
				DexModule::add_liquidity(Origin::signed(ALICE), JUSD, DNAR, 0, 100_000_000, false),
				Error::<Runtime>::InvalidLiquidityIncrement
			);

			assert_eq!(DexModule::get_liquidity(JUSD, DNAR), (0, 0));
			assert_eq!(Tokens::free_balance(JUSD, &DexModule::account_id()), 0);
			assert_eq!(Tokens::free_balance(DNAR, &DexModule::account_id()), 0);
			assert_eq!(
				Tokens::free_balance(JUSD_DNAR_PAIR.get_dex_share_currency_id().unwrap(), &ALICE),
			...
			assert_eq!(
				DexModule::get_liquidity(JUSD, DNAR),
				(5_000_000_000_000, 1_000_000_000_000)
			);
			assert_eq!(Tokens::free_balance(JUSD, &DexModule::account_id()), 5_000_000_000_000);
			...
			assert_eq!(Tokens::free_balance(JUSD, &DexModule::account_id()), 45_000_000_000_000);
...
fn remove_liquidity_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Tokens::free_balance(JUSD, &DexModule::account_id()), 1_000_000_000_000);
...
fn do_swap_with_exact_supply_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				DexModule::get_liquidity(JUSD, JCHF),
			...
			assert_eq!(Tokens::free_balance(JCHF, &DexModule::account_id()), 10_000_000_000);
			assert_eq!(Tokens::free_balance(JUSD, &BOB), 1_000_000_000_000_000_000);
			assert_eq!(Tokens::free_balance(DNAR, &BOB), 1_000_000_000_000_000_000);
			...
			assert_noop!(
				DexModule::do_swap_with_exact_supply(&BOB, &[DNAR, JUSD, JCHF, DNAR], 100_000_000_000_000, 0, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::do_swap_with_exact_supply(&BOB, &[DNAR, DNAR], 100_000_000_000_000, 0, None),
				Error::<Runtime>::MustBeEnabled,
			);
			...
			assert_eq!(Tokens::free_balance(JCHF, &DexModule::account_id()), 4_469_336_163);
			assert_eq!(Tokens::free_balance(JUSD, &BOB), 1_000_248_743_718_592_964);
...
fn do_swap_with_exact_target_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::do_swap_with_exact_target(&BOB, &[DNAR, DNAR], 250_000_000_000_000, 200_000_000_000_000, None),
				Error::<Runtime>::MustBeEnabled,
			);
			...
			assert_eq!(Tokens::free_balance(JCHF, &DexModule::account_id()), 5_000_000_000);
			assert_eq!(Tokens::free_balance(JUSD, &BOB), 1_000_250_000_000_000_000);
...
fn initialize_added_liquidity_pools_genesis_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(JUSD, DNAR), (1000000, 2000000));
			assert_eq!(Tokens::free_balance(JUSD, &DexModule::account_id()), 2000000);
