...
use support::{SettinDexManager, ExchangeRateProvider, Price, PriceProvider};
...
pub mod module {
	...
	pub trait Config: frame_system::Config {
		...
		#[pallet::constant]
		/// The stable currency id, it should be JUSD in Setheum.
		...
		type SettinDex: SettinDexManager<Self::AccountId, CurrencyId, Balance>;
...
impl<T: Config> PriceProvider<CurrencyId> for Pallet<T> {
	...
	fn get_price(currency_id: CurrencyId) -> Option<Price> {
		let maybe_feed_price = if currency_id == T::GetStableCurrencyId::get() {
			...
			// if is setheum-abha liquid currency, return the product of staking currency price and
