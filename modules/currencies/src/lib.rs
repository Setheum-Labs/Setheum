...
use support::{SEVMBridge, InvokeContext};
...
pub mod module {
	...
	pub trait Config: frame_system::Config {
		...
		type SEVMBridge: SEVMBridge<Self::AccountId, BalanceOf<Self>>;
	}
...
impl<T: Config> MultiCurrency<T::AccountId> for Pallet<T> {
	...
	fn total_issuance(currency_id: Self::CurrencyId) -> Self::Balance {
		match currency_id {
			CurrencyId::ERC20(contract) => T::SEVMBridge::total_supply(InvokeContext {
				contract,
		...
		match currency_id {
			CurrencyId::ERC20(contract) => {
				if let Some(address) = T::AddressMapping::get_evm_address(&who) {
					...
					return T::SEVMBridge::balance_of(context, address).unwrap_or_default();
				}
				Default::default()
			}
	...
	fn ensure_can_withdraw(currency_id: Self::CurrencyId, who: &T::AccountId, amount: Self::Balance) -> DispatchResult {
		match currency_id {
			CurrencyId::ERC20(contract) => {
				...
				let balance = T::SEVMBridge::balance_of(
					InvokeContext {
						contract,
						sender: Default::default(),
						origin: Default::default(),
	...
	fn transfer(
		...
		match currency_id {
			CurrencyId::ERC20(contract) => {
				...
				let origin = T::SEVMBridge::get_origin().unwrap_or_default();
...
impl<T: Config> MultiReservableCurrency<T::AccountId> for Pallet<T> {
	...
	fn reserved_balance(currency_id: Self::CurrencyId, who: &T::AccountId) -> Self::Balance {
		match currency_id {
			CurrencyId::ERC20(contract) => {
				if let Some(address) = T::AddressMapping::get_evm_address(&who) {
					return T::SEVMBridge::balance_of(
						InvokeContext {
							contract,
							sender: Default::default(),
							origin: Default::default(),
		...
		match currency_id {
			CurrencyId::ERC20(contract) => {
				...
				if let Some(address) = T::AddressMapping::get_evm_address(&who) {
					...
					let reserved_balance = T::SEVMBridge::balance_of(
						InvokeContext {
							contract,
							sender: Default::default(),
							origin: Default::default(),
					...
					return match T::SEVMBridge::transfer(
						InvokeContext {
							contract,
							sender,
							origin: address,
	...
	fn repatriate_reserved(
		...
		match currency_id {
			CurrencyId::ERC20(contract) => {
				...
				let slashed_reserved_balance = T::SEVMBridge::balance_of(
					InvokeContext {
						contract,
						sender: Default::default(),
						origin: Default::default(),
				...
				match status {
					BalanceStatus::Free => T::SEVMBridge::transfer(
						InvokeContext {
							contract,
					...
					BalanceStatus::Reserved => T::SEVMBridge::transfer(
						InvokeContext {
							contract,
							sender: slashed_reserve_address,
							origin: slashed_address,
