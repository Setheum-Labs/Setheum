...
use mock::{
	AccountId, BlockWeights, Call, Currencies, SettinDex, ExtBuilder, Origin, Runtime, TransactionPayment, DNAR, ALICE,
	JUSD, BOB, DOT,
};
...
const CALL: &<Runtime as frame_system::Config>::Call =
	&Call::Currencies(module_currencies::Call::transfer(BOB, JUSD, 12));
...
fn charges_fee_when_validate_and_native_is_not_enough() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(<Currencies as MultiCurrency<_>>::transfer(JUSD, &ALICE, &BOB, 1000));
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(DNAR, &BOB), 0);
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(JUSD, &BOB), 1000);
		...
		assert_eq!(SettinDex::get_liquidity_pool(DNAR, JUSD), (10000, 1000));
		...
		assert_eq!(Currencies::free_balance(JUSD, &BOB), 749);
		assert_eq!(SettinDex::get_liquidity_pool(DNAR, JUSD), (10000 - 2000, 1251));
	});
}
...
fn set_default_fee_token_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(TransactionPayment::default_fee_currency_id(&ALICE), Some(JUSD));
...
fn charge_fee_by_default_fee_token() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(SettinDex::get_liquidity_pool(DNAR, JUSD), (10000, 1000));
		assert_eq!(SettinDex::get_liquidity_pool(DOT, JUSD), (100, 1000));
		assert_ok!(TransactionPayment::set_default_fee_token(
			Origin::signed(BOB),
			Some(DOT)
		));
		...
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(DNAR, &BOB), 0);
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(JUSD, &BOB), 0);
		...
		assert_eq!(Currencies::free_balance(DNAR, &BOB), 0);
		assert_eq!(Currencies::free_balance(JUSD, &BOB), 0);
		assert_eq!(Currencies::free_balance(DOT, &BOB), 100 - 34);
		assert_eq!(SettinDex::get_liquidity_pool(DNAR, JUSD), (10000 - 2000, 1251));
		assert_eq!(SettinDex::get_liquidity_pool(DOT, JUSD), (100 + 34, 1000 - 251));
	});
}
