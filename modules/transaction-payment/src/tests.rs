...
use mock::{
	AccountId, BlockWeights, Call, Currencies, DEXModule, ExtBuilder, Origin, Runtime, TransactionPayment, DNAR, ALICE,
	AUSD, BOB, DOT,
};
...
fn charges_fee_when_validate_and_native_is_not_enough() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DEXModule::get_liquidity_pool(DNAR, AUSD), (10000, 1000));
		...
		assert_eq!(DEXModule::get_liquidity_pool(DNAR, AUSD), (10000 - 2000, 1251));
	});
}
