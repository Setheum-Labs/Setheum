...
use support::{SEVMBridge, InvokeContext, Ratio};
...
pub struct MockSEVMBridge;
impl<AccountId, Balance> SEVMBridge<AccountId, Balance> for MockSEVMBridge
where
	AccountId: Default,
	Balance: Default,
{
...
impl module_currencies::Config for Runtime {
	...
	type SEVMBridge = MockSEVMBridge;
}
...
parameter_types! {
	...
	pub EnabledTradingPairs : Vec<TradingPair> = vec![TradingPair::new(JUSD, DNAR), TradingPair::new(JUSD, DOT)];
}
...
impl settindex::Config for Runtime {
	...
	type SettinDexIncentives = ();
	type WeightInfo = ();
	type ListingOrigin = frame_system::EnsureSignedBy<Zero, AccountId>;
}

parameter_types! {
	pub AllNonNativeCurrencyIds: Vec<CurrencyId> = vec![JUSD, DOT];
...
impl Default for ExtBuilder {
	fn default() -> Self {
		Self {
			endowed_accounts: vec![(ALICE, JUSD, 10000), (ALICE, DOT, 1000)],
			base_weight: 0,
			byte_fee: 2,
			weight_to_fee: 1,
		}
