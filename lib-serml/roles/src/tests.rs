...
use slixon_utils::{Error as UtilsError};
...
fn create_role_should_work() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_eq!(role.channel_id, SPACE1);
...
#[test]
fn create_role_should_fail_with_channel_not_found() {
    ExtBuilder::build().execute_with(|| {
        assert_noop!(
            _create_role(
                None, // From ACCOUNT1
...
fn grant_role_should_work() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_eq!(Roles::role_ids_by_user_in_channel(user, SPACE1), vec![ROLE1]);
    });
}
...
fn revoke_role_should_work() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert!(Roles::role_ids_by_user_in_channel(user, SPACE1).is_empty());
    });
}
...
fn delete_role_should_work() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert!(Roles::users_by_role_id(ROLE1).is_empty());
        assert!(Roles::role_ids_by_channel_id(SPACE1).is_empty());
        assert!(Roles::role_ids_by_user_in_channel(User::Account(ACCOUNT2), SPACE1).is_empty());
        assert_eq!(Roles::next_role_id(), ROLE2);
    });
}
...
fn delete_role_should_work_with_a_few_roles() {
    ExtBuilder::build_with_a_few_roles_granted_to_account2().execute_with(|| {
        ...
        assert!(Roles::role_by_id(ROLE1).is_none());
        assert!(Roles::users_by_role_id(ROLE1).is_empty());
        assert_eq!(Roles::role_ids_by_channel_id(SPACE1), vec![ROLE2]);
        assert_eq!(Roles::role_ids_by_user_in_channel(User::Account(ACCOUNT2), SPACE1), vec![ROLE2]);
