...
//! and grant them to users (accounts or channel ids) within a given channel.
//! 
//! For example if you want to create a channel that enables editors in a similar way to Medium,
...
use module_support::{
    PermissionChecker, ChannelFollowsProvider, ChannelForRolesProvider,
    moderation::{IsAccountBlocked, IsContentBlocked},
};
use slixon_permissions::{Module as Permissions, ChannelPermission, ChannelPermissionSet};
use slixon_utils::{Module as Utils, Error as UtilsError, ChannelId, User, WhoAndWhen, Content};
...
/// Information about a role's permissions, its' containing channel, and its' content.
...
pub struct Role<T: Trait> {
    ...
    /// An id of a channel that contains this role.
    ...
    /// only within the channel containing this role
...
pub struct RoleUpdate {
    ...
    pub permissions: Option<ChannelPermissionSet>,
}
    ...
    + slixon_utils::Trait
{
    ...
    type Channels: ChannelForRolesProvider<AccountId=Self::AccountId>;
...
decl_event!(
    ...
    {
        RoleCreated(AccountId, ChannelId, RoleId),
...
decl_error! {
    pub enum Error for Module<T: Trait> {
        ...
        /// Account does not have permission to manage roles in this channel.
...
decl_storage! {
    trait Store for Module<T: Trait> as PermissionsModule {
        ...
        /// Get a list of all users (account or channel ids) that a given role has been granted to.
        pub UsersByRoleId get(fn users_by_role_id):
            map hasher(twox_64_concat) RoleId => Vec<User<T::AccountId>>;

        /// Get a list of all role ids available in a given channel.
        pub RoleIdsByChannelId get(fn role_ids_by_channel_id):
            map hasher(twox_64_concat) ChannelId => Vec<RoleId>;

        /// Get a list of all role ids owned by a given user (account or channel id)
        /// within a given channel.
        pub RoleIdsByUserInChannel get(fn role_ids_by_user_in_channel): double_map
...
decl_module! {
  pub struct Module<T: Trait> for enum Call where origin: T::Origin {
    ...
    /// Create a new role, with a list of permissions, within a given channel.
    ///
    /// `content` can optionally contain additional information associated with a role, 
    /// such as a name, description, and image for a role. This may be useful for end users.
    ///
    /// Only the channel owner or a user with `ManageRoles` permission can call this dispatch.
    ...
    pub fn create_role(
      ...
      ensure!(T::IsContentBlocked::is_allowed_content(content.clone(), channel_id), UtilsError::<T>::ContentIsBlocked);

      Self::ensure_role_manager(who.clone(), channel_id)?;
      
      let permissions_set = permissions.into_iter().collect();
      let new_role = Role::<T>::new(who.clone(), channel_id, time_to_live, content, permissions_set)?;
      ...
      RoleIdsByChannelId::mutate(channel_id, |role_ids| { role_ids.push(new_role.id) });

      Self::deposit_event(RawEvent::RoleCreated(who, channel_id, new_role.id));
      Ok(())
    }
    ...
    pub fn update_role(origin, role_id: RoleId, update: RoleUpdate) -> DispatchResult {
      ...
      Self::ensure_role_manager(who.clone(), role.channel_id)?;
      ...
      if let Some(content) = update.content {
        if content != role.content {
          Utils::<T>::is_valid_content(content.clone())?;
          ensure!(T::IsContentBlocked::is_allowed_content(content.clone(), role.channel_id), UtilsError::<T>::ContentIsBlocked);
    ...
    pub fn delete_role(origin, role_id: RoleId) -> DispatchResult {
      ...
      let role_idx_by_channel_opt = Self::role_ids_by_channel_id(role.channel_id).iter()
        .position(|x| { *x == role_id });

      if let Some(role_idx) = role_idx_by_channel_opt {
        RoleIdsByChannelId::mutate(role.channel_id, |n| { n.swap_remove(role_idx) });
      }
    ...
    pub fn grant_role(origin, role_id: RoleId, users: Vec<User<T::AccountId>>) -> DispatchResult {
      ...
      for user in users_set.iter() {
        ...
        if !Self::role_ids_by_user_in_channel(user.clone(), role.channel_id).contains(&role_id) {
          <RoleIdsByUserInChannel<T>>::mutate(user.clone(), role.channel_id, |roles| { roles.push(role_id); })
        }
      }
