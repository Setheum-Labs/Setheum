...
fn enable_average_price_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexOracle::enable_average_price(Origin::signed(0), SETUSD, SETM, 0),
			BadOrigin
		);
		assert_noop!(
			DexOracle::enable_average_price(Origin::signed(1), SETUSD, LP_SETUSD_SETM, 0),
			Error::<Runtime>::InvalidCurrencyId
		);
		assert_noop!(
			DexOracle::enable_average_price(Origin::signed(1), SETUSD, SETM, 0),
			Error::<Runtime>::IntervalIsZero
		);
		assert_noop!(
			DexOracle::enable_average_price(Origin::signed(1), SETUSD, SETM, 12000),
			Error::<Runtime>::InvalidPool
		);

		set_pool(&SETUSDSETMPair::get(), 1_000, 100);
		assert_eq!(
			DexOracle::cumulatives(SETUSDSETMPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
		assert_eq!(DexOracle::average_prices(SETUSDSETMPair::get()), None);

		assert_ok!(DexOracle::enable_average_price(Origin::signed(1), SETUSD, SETM, 12000));
		assert_eq!(
			DexOracle::cumulatives(SETUSDSETMPair::get()),
			(U256::from(0), U256::from(0), 1000)
		);
		assert_eq!(
			DexOracle::average_prices(SETUSDSETMPair::get()),
			Some((
...
fn disable_average_price_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(DexOracle::enable_average_price(Origin::signed(1), SETUSD, SETM, 1000));
		assert_eq!(
			DexOracle::cumulatives(SETUSDSETMPair::get()),
		...
		assert_noop!(
			DexOracle::disable_average_price(Origin::signed(0), SETUSD, SETM),
			BadOrigin
		);
		assert_noop!(
			DexOracle::disable_average_price(Origin::signed(1), SETUSD, LP_SETUSD_SETM),
			Error::<Runtime>::InvalidCurrencyId
		);
		assert_noop!(
			DexOracle::disable_average_price(Origin::signed(1), SETM, SETM),
			Error::<Runtime>::AveragePriceMustBeEnabled
		);

		assert_ok!(DexOracle::disable_average_price(Origin::signed(1), SETUSD, SETM));
		assert_eq!(
			DexOracle::cumulatives(SETUSDSETMPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
...
fn update_average_price_interval_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexOracle::update_average_price_interval(Origin::signed(0), SETUSD, SETM, 0),
			BadOrigin
		);
		assert_noop!(
			DexOracle::update_average_price_interval(Origin::signed(1), SETUSD, LP_SETUSD_SETM, 0),
			Error::<Runtime>::InvalidCurrencyId
		);
		assert_noop!(
			DexOracle::update_average_price_interval(Origin::signed(1), SETM, SETM, 0),
			Error::<Runtime>::AveragePriceMustBeEnabled
		);
		assert_noop!(
			DexOracle::update_average_price_interval(Origin::signed(1), SETUSD, SETM, 0),
			Error::<Runtime>::IntervalIsZero
		);
...
fn try_update_cumulative_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		DexOracle::try_update_cumulative(&SETUSDSETMPair::get(), 500, 200);
		assert_eq!(
			DexOracle::cumulatives(SETUSDSETMPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
		...
		DexOracle::try_update_cumulative(&SETUSDSETMPair::get(), 1_000, 100);
		assert_eq!(
			DexOracle::cumulatives(SETUSDSETMPair::get()),
			(
		...
		assert_eq!(
			DexOracle::cumulatives(SETMSETMPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
		DexOracle::try_update_cumulative(&SETMSETMPair::get(), 500, 200);
		assert_eq!(
			DexOracle::cumulatives(SETMSETMPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
...
fn on_initialize_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Timestamp::now(), 0);
		set_pool(&SETUSDSETMPair::get(), 1000, 100);
		assert_ok!(DexOracle::enable_average_price(Origin::signed(1), SETUSD, SETM, 1000));
		assert_eq!(
			DexOracle::cumulatives(SETUSDSETMPair::get()),
			(U256::from(0), U256::from(0), 0)
		...
		set_pool(&SETMSETMPair::get(), 1000, 1000);
		assert_ok!(DexOracle::enable_average_price(Origin::signed(1), SETM, SETM, 2000));
		assert_eq!(
			DexOracle::cumulatives(SETMSETMPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
		assert_eq!(
			DexOracle::average_prices(SETMSETMPair::get()),
			Some((
		...
		// elapsed time is lt the update interval of SETUSD/SETM, update average price of SETUSD/SETM after try
		// update cumulatives.
		...
		// elapsed time is lt the update interval of SETM/SETM, update average price of SETM/SETM after try
		// update cumulatives.
		set_pool(&SETMSETMPair::get(), 1000, 2000);
		...
		set_pool(&SETUSDSETMPair::get(), 2000, 100);
		set_pool(&SETMSETMPair::get(), 1000, 4000);
		...
		DexOracle::try_update_cumulative(&SETUSDSETMPair::get(), 100, 100);
		DexOracle::try_update_cumulative(&SETMSETMPair::get(), 2000, 200);
		assert_eq!(
			DexOracle::cumulatives(SETUSDSETMPair::get()),
			(
		...
		// update average prices of SETUSD/SETM and SETM/SETM
		set_pool(&SETUSDSETMPair::get(), 1000, 100);
		set_pool(&SETMSETMPair::get(), 1000, 1000);
...
fn dex_price_providers_work() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(CurrentDEXPriceProvider::<Runtime>::get_relative_price(SETUSD, SETM), None);
		assert_eq!(CurrentDEXPriceProvider::<Runtime>::get_relative_price(SETM, SETUSD), None);
		assert_eq!(AverageDEXPriceProvider::<Runtime>::get_relative_price(SETUSD, SETM), None);
		assert_eq!(AverageDEXPriceProvider::<Runtime>::get_relative_price(SETM, SETUSD), None);
		assert_eq!(
			PriorityAverageDEXPriceProvider::<Runtime>::get_relative_price(SETUSD, SETM),
			None
		);
		assert_eq!(
			PriorityAverageDEXPriceProvider::<Runtime>::get_relative_price(SETM, SETUSD),
			None
		);

		set_pool(&SETUSDSETMPair::get(), 1_000, 100);
		assert_eq!(
			CurrentDEXPriceProvider::<Runtime>::get_relative_price(SETUSD, SETM),
			Some(ExchangeRate::saturating_from_rational(1, 10))
		);
		assert_eq!(
			CurrentDEXPriceProvider::<Runtime>::get_relative_price(SETM, SETUSD),
		...
		assert_eq!(
			AverageDEXPriceProvider::<Runtime>::get_relative_price(SETUSD, SETM),
			Some(ExchangeRate::saturating_from_rational(2, 10))
		);
		assert_eq!(
			AverageDEXPriceProvider::<Runtime>::get_relative_price(SETM, SETUSD),
		...
		set_pool(&SETUSDSETMPair::get(), 300, 100);
		assert_eq!(
			CurrentDEXPriceProvider::<Runtime>::get_relative_price(SETUSD, SETM),
