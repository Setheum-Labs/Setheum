...
fn collateral_auction_methods() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AuctionManagerModule::new_collateral_auction(&ALICE, BTC, SETUSD, 10, 0));
...
fn new_collateral_auction_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			AuctionManagerModule::new_collateral_auction(&ALICE, BTC, SETUSD, 0, 100),
			Error::<Runtime>::InvalidAmount,
		);

		assert_ok!(AuctionManagerModule::new_collateral_auction(&ALICE, BTC, SETUSD, 10, 100));
		Event::AuctionManagerModule(crate::Event::NewCollateralAuction(
			0, BTC, SETUSD, 10, 100,
		));
		...
		assert_noop!(
			AuctionManagerModule::new_collateral_auction(&ALICE, BTC, SETUSD, Balance::max_value(), Balance::max_value()),
			Error::<Runtime>::InvalidAmount,
		);
	});
}
...
fn collateral_auction_bid_handler_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(CDPTreasuryModule::surplus_pool(SETUSD), 5);
		...
		assert_eq!(CDPTreasuryModule::surplus_pool(SETUSD), 10);
		assert_eq!(Tokens::free_balance(SETUSD, &BOB), 1000);
		...
		assert_eq!(CDPTreasuryModule::surplus_pool(SETUSD), 100);
...
fn collateral_auction_end_handler_without_bid() {
	ExtBuilder::default().build().execute_with(|| {
		...
		Event::AuctionManagerModule(crate::Event::CancelAuction(0));
...
fn collateral_auction_end_handler_in_reverse_stage() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(CDPTreasuryModule::surplus_pool(SETUSD), 200);
		...
		Event::AuctionManagerModule(crate::Event::CollateralAuctionDealt(
			0, BTC, 50, BOB, 200,
		));
...
fn collateral_auction_end_handler_by_dex_which_target_not_zero() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(CDPTreasuryModule::surplus_pool(SETUSD), 20);
		...
		Event::AuctionManagerModule(crate::Event::DEXTakeCollateralAuction(
			0, BTC, 100, 500,
		));
		...
		assert_eq!(Tokens::free_balance(BTC, &BOB), 1000);
		assert_eq!(Tokens::free_balance(SETUSD, &BOB), 1000);
		assert_eq!(Tokens::free_balance(SETUSD, &ALICE), 1300);
		assert_eq!(CDPTreasuryModule::debit_pool(SETUSD), 320);
		assert_eq!(CDPTreasuryModule::surplus_pool(SETUSD), 520);
