...
use slixon_utils::{bool_to_option, PostId, rpc::{FlatContent, FlatWhoAndWhen, ShouldSkip}, ChannelId};
...
impl<T: Trait> From<Post<T>> for FlatPost<T::AccountId, T::BlockNumber> {
    fn from(from: Post<T>) -> Self {
        let Post {
            ...
            extension, channel_id, content, hidden, replies_count,
...
impl<T: Trait> Module<T> {
    ...
    fn get_posts_slice_by_channel_id<F: FnMut(&Post<T>) -> bool>(
        channel_id: ChannelId,
        offset: u64,
        limit: u16,
        filter: F,
    ) -> Vec<FlatPost<T::AccountId, T::BlockNumber>> {
        let mut post_ids: Vec<PostId> = Self::post_ids_by_channel_id(channel_id);
        post_ids.reverse();

        Self::get_posts_by_ids_with_filter(post_ids, offset, limit, filter)
    }

    pub fn get_public_posts_by_channel_id(
        channel_id: ChannelId,
        offset: u64,
        limit: u16,
    ) -> Vec<FlatPost<T::AccountId, T::BlockNumber>> {
        if let Ok(channel) = Channels::<T>::require_channel(channel_id) {
            return Self::get_posts_slice_by_channel_id(channel.id, offset, limit, |post| post.is_public());
        }

        vec![]
    }

    pub fn get_unlisted_posts_by_channel_id(
        channel_id: ChannelId,
        offset: u64,
        limit: u16,
    ) -> Vec<FlatPost<T::AccountId, T::BlockNumber>> {
        if let Ok(channel) = Channels::<T>::require_channel(channel_id) {
            return Self::get_posts_slice_by_channel_id(channel.id, offset, limit, |post| post.is_unlisted());
        }
    ...
    fn get_post_ids_by_channel<F: FnMut(&Post<T>) -> bool>(channel_id: ChannelId, mut filter: F) -> Vec<PostId> {
        Self::post_ids_by_channel_id(channel_id)
            .iter()
    ...
    pub fn get_public_post_ids_by_channel_id(channel_id: ChannelId) -> Vec<PostId> {
        let public_channel = Channels::<T>::require_channel(channel_id).ok().filter(|channel| channel.is_public());
        if public_channel.is_some() {
            return Self::get_post_ids_by_channel(channel_id, |post| post.is_public());
        }

        vec![]
    }

    pub fn get_unlisted_post_ids_by_channel_id(channel_id: ChannelId) -> Vec<PostId> {
        let unlisted_channel = Channels::<T>::require_channel(channel_id).ok().filter(|channel| !channel.is_public());
        if unlisted_channel.is_some() {
            return Self::get_post_ids_by_channel(channel_id, |post| !post.is_public());
        }
    ...
    pub fn get_feed(account: T::AccountId, offset: u64, limit: u16) -> Vec<FlatPost<T::AccountId, T::BlockNumber>> {
        let mut post_ids: Vec<PostId> = ChannelFollows::<T>::channels_followed_by_account(account)
            .iter()
            .flat_map(Self::post_ids_by_channel_id)
            .collect();
