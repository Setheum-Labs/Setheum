...
use slixon_utils::{ChannelId, remove_from_vec};
...
impl<T: Trait> Post<T> {
    ...
    pub fn get_channel_id(&self) -> Result<ChannelId, DispatchError> {
        Self::try_get_channel_id(self).ok_or_else(|| Error::<T>::PostHasNoChannelId.into())
    }

    pub fn try_get_channel_id(&self) -> Option<ChannelId> {
        if let Ok(root_post) = self.get_root_post() {
            return root_post.channel_id;
        }

        None
    }

    pub fn get_channel(&self) -> Result<Channel<T>, DispatchError> {
        let root_post = self.get_root_post()?;
        let channel_id = root_post.channel_id.ok_or(Error::<T>::PostHasNoChannelId)?;
        Channels::require_channel(channel_id)
    }

    pub fn try_get_channel(&self) -> Option<Channel<T>> {
        if let Ok(root_post) = self.get_root_post() {
            return root_post.channel_id.and_then(|channel_id| Channels::require_channel(channel_id).ok());
        }

        None
    }

    // TODO use macros to generate inc/dec fns for Channel, Post.
...
impl<T: Trait> Module<T> {
    ...
    pub fn ensure_account_can_update_post(
        ...
        let permission_to_check: ChannelPermission;
        let permission_error: DispatchError;

        if is_comment {
          if is_owner {
            permission_to_check = ChannelPermission::UpdateOwnComments;
          ...
          if is_owner {
            permission_to_check = ChannelPermission::UpdateOwnPosts;
            permission_error = Error::<T>::NoPermissionToUpdateOwnPosts.into();
          } else {
            permission_to_check = ChannelPermission::UpdateAnyPost;
    ...
    pub fn require_post(post_id: ChannelId) -> Result<Post<T>, DispatchError> {
    ...
    pub(crate) fn create_sharing_post(
        ...
        // Check if it's allowed to share a post from the channel of original post.
        Channels::ensure_account_has_channel_permission(
            creator.clone(),
            &original_post.get_channel()?,
            ChannelPermission::Share,
    ...
    fn mutate_posts_count_on_channel<F: FnMut(&mut u32) + Copy> (
        channel_id: ChannelId,
        post: &Post<T>,
        mut f: F
    ) -> DispatchResult {
        Channels::<T>::mutate_channel_by_id(channel_id, |channel: &mut Channel<T>| {
            f(&mut channel.posts_count);
            if post.hidden {
                f(&mut channel.hidden_posts_count);
            }
        }).map(|_| ())
    }

    pub(crate) fn move_post_to_channel(
        editor: T::AccountId,
        post: &mut Post<T>,
        new_channel_id: ChannelId
    ) -> DispatchResult {
        let old_channel_id_opt = post.try_get_channel_id();
        let new_channel = Channels::<T>::require_channel(new_channel_id)?;

        ensure!(
            T::IsAccountBlocked::is_allowed_account(editor.clone(), new_channel_id),
            UtilsError::<T>::AccountIsBlocked
        );
        Channels::ensure_account_has_channel_permission(
            editor,
            &new_channel,
            ChannelPermission::CreatePosts,
            Error::<T>::NoPermissionToCreatePosts.into()
        )?;
        ensure!(
            T::IsPostBlocked::is_allowed_post(post.id, new_channel_id),
            UtilsError::<T>::PostIsBlocked
        );
        ensure!(
            T::IsContentBlocked::is_allowed_content(post.content.clone(), new_channel_id),
        ...
        match post.extension {
            PostExtension::RegularPost | PostExtension::SharedPost(_) => {
                ...
                if let Some(old_channel_id) = old_channel_id_opt {

                    // Decrease the number of posts on the old channel
                    Self::mutate_posts_count_on_channel(
                        old_channel_id,
                        post,
                        |counter| *counter = counter.saturating_sub(1)
                    )?;

                    // Decrease a score on the old channel
                    Channels::<T>::mutate_channel_by_id(
                        old_channel_id,
                        |channel| channel.score = channel.score.saturating_sub(post.score)
                    )?;

                    PostIdsByChannelId::mutate(old_channel_id, |post_ids| remove_from_vec(post_ids, post.id));
                }

                // Increase the number of posts on the new channel
                Self::mutate_posts_count_on_channel(
                    new_channel_id,
                    post,
                ...
                // Increase a score on the new channel
                Channels::<T>::mutate_channel_by_id(
                    new_channel_id,
                    |channel| channel.score = channel.score.saturating_add(post.score)
                )?;

                PostIdsByChannelId::mutate(new_channel_id, |post_ids| post_ids.push(post.id));

                post.channel_id = Some(new_channel_id);
                PostById::<T>::insert(post.id, post);

                Ok(())
            },
            _ => fail!(Error::<T>::CannotUpdateChannelIdOnComment),
        }
    }

    pub fn delete_post_from_channel(post_id: PostId) -> DispatchResult {
        ...
        if let PostExtension::Comment(comment_ext) = post.extension {
            ...
            // Subtract the weight of CreateComment from the root post and its channel
            ...
            let channel_id = post.get_channel_id()?;

            // Decrease the number of posts on the channel
            Self::mutate_posts_count_on_channel(
                channel_id,
                &post,
                |counter| *counter = counter.saturating_sub(1)
            ...
            PostIdsByChannelId::mutate(channel_id, |post_ids| remove_from_vec(post_ids, post_id));
        }
