...
fn name_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				EvmCurrencyIdMapping::<Runtime>::name(CurrencyId::DexShare(DexShare::Token(TokenSymbol::DNAR), DexShare::Erc20(erc20_address_not_exists()))),
				None
			);
...
fn symbol_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				EvmCurrencyIdMapping::<Runtime>::symbol(CurrencyId::Token(TokenSymbol::DNAR)),
				Some(b"DNAR".to_vec())
			);
			assert_eq!(
...
fn decimals_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				EvmCurrencyIdMapping::<Runtime>::decimals(CurrencyId::Token(TokenSymbol::DNAR)),
				Some(12)
			);
			assert_eq!(
...
fn encode_evm_address_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				EvmCurrencyIdMapping::<Runtime>::encode_evm_address(CurrencyId::Token(TokenSymbol::DNAR)),
		...
		.execute_with(|| {
			...
			assert_eq!(
				EvmCurrencyIdMapping::<Runtime>::decode_evm_address(
					EvmCurrencyIdMapping::<Runtime>::encode_evm_address(CurrencyId::Token(TokenSymbol::DNAR)).unwrap()
				),
				Some(CurrencyId::Token(TokenSymbol::DNAR))
			);
			assert_eq!(
				EvmCurrencyIdMapping::<Runtime>::decode_evm_address(
			...
			// CurrencyId::DexShare(DexShare::Token(TokenSymbol::DNAR),
			// DexShare::Erc20(erc20_address_not_exists()))
			assert_eq!(
				EvmCurrencyIdMapping::<Runtime>::decode_evm_address(
