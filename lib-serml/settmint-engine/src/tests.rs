...
fn settle_settmint_has_standard_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(SettmintEngineModule::adjust_position(&ALICE, SETT, 100, 0));
		assert_eq!(Currencies::free_balance(SETT, &ALICE), 900);
		assert_eq!(SettersModule::positions(SETT, ALICE).standard, 0);
		assert_eq!(SettersModule::positions(SETT, ALICE).reserve, 100);
		assert_noop!(
			SettmintEngineModule::settle_settmint_has_standard(ALICE, SETT),
			Error::<Runtime>::NoStandardValue,
		);
		assert_ok!(SettmintEngineModule::adjust_position(&ALICE, SETT, 0, 50));
		assert_eq!(SettersModule::positions(SETT, ALICE).standard, 50);
		assert_eq!(SerpTreasuryModule::standard_pool(), 0);
		assert_eq!(SerpTreasuryModule::total_reserves(SETT), 0);
		assert_ok!(SettmintEngineModule::settle_settmint_has_standard(ALICE, SETT));

		let settle_settmint_in_standard_event = Event::settmint_engine(crate::Event::SettleSettmintInStandard(SETT, ALICE));
		assert!(System::events()
			.iter()
		...
		assert_eq!(SerpTreasuryModule::total_reserves(SETT), 50);

		assert_noop!(
			SettmintEngineModule::settle(Origin::none(), SETT, ALICE),
