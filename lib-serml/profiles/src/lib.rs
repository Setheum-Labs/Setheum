...
use slixon_utils::{Module as Utils, WhoAndWhen, Content};
...
impl <T: Trait> SocialAccount<T> {
    ...
    pub fn inc_following_channels(&mut self) {
        self.following_channels_count = self.following_channels_count.saturating_add(1);
    }

    pub fn dec_following_channels(&mut self) {
        self.following_channels_count = self.following_channels_count.saturating_sub(1);
    }
}
