...
use mock::{
	DexModule, Event, ExtBuilder, ListingOrigin, Origin, Runtime, System, Tokens, 
...
fn enable_new_trading_pair_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::enable_trading_pair(Origin::signed(ALICE), USDJ, DNAR),
			BadOrigin
		);

		assert_eq!(
			DexModule::trading_pair_statuses(USDJ_DNAR_PAIR),
		...
		assert_noop!(
			DexModule::enable_trading_pair(Origin::signed(ListingOrigin::get()), DNAR, USDJ),
...
fn disable_enabled_trading_pair_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::disable_trading_pair(Origin::signed(ALICE), USDJ, DNAR),
			BadOrigin
		);
		...
		assert_noop!(
			DexModule::disable_trading_pair(Origin::signed(ListingOrigin::get()), USDJ, DNAR),
...
fn disable_provisioning_trading_pair_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				DexModule::provisioning_pool(USDJ_DNAR_PAIR, ALICE),
				(5_000_000_000_000u128, 0)
			);
			assert_eq!(
				DexModule::provisioning_pool(USDJ_DNAR_PAIR, BOB),
			...
			assert_eq!(DexModule::provisioning_pool(USDJ_DNAR_PAIR, ALICE), (0, 0));
			assert_eq!(DexModule::provisioning_pool(USDJ_DNAR_PAIR, BOB), (0, 0));
			assert_eq!(
				DexModule::trading_pair_statuses(USDJ_DNAR_PAIR),
				TradingPairStatus::<_, _>::NotEnabled
			);
...
fn get_liquidity_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DexModule::liquidity_pool(USDJ_DNAR_PAIR), (1000, 20));
		assert_eq!(DexModule::get_liquidity(USDJ, DNAR), (1000, 20));
		assert_eq!(DexModule::get_liquidity(DNAR, USDJ), (20, 1000));
	});
}
...
fn get_target_amounts_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			LiquidityPool::<Runtime>::insert(USDJ_CHFJ_PAIR, (100000, 10));
			assert_noop!(
				DexModule::get_target_amounts(&vec![DNAR], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![DNAR, USDJ, CHFJ, EURJ], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_eq!(
				DexModule::get_target_amounts(&vec![DNAR, USDJ], 10000, None),
				Ok(vec![10000, 24874])
			);
			assert_eq!(
			...
			assert_noop!(
				DexModule::get_target_amounts(&vec![DNAR, USDJ], 10000, Ratio::checked_from_rational(49, 100)),
				Error::<Runtime>::ExceedPriceImpactLimit,
			);
			assert_eq!(
				DexModule::get_target_amounts(&vec![DNAR, USDJ, CHFJ], 10000, None),
...
fn get_supply_amounts_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::get_supply_amounts(&vec![DNAR, USDJ, CHFJ, EURJ], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_eq!(
				DexModule::get_supply_amounts(&vec![DNAR, USDJ], 24874, None),
				Ok(vec![10000, 24874])
			);
			assert_eq!(
			...
			assert_noop!(
				DexModule::get_supply_amounts(&vec![DNAR, USDJ], 25000, Ratio::checked_from_rational(49, 100)),
				Error::<Runtime>::ExceedPriceImpactLimit,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![DNAR, USDJ, CHFJ], 10000, None),
				Error::<Runtime>::ZeroSupplyAmount,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![DNAR, CHFJ], 10000, None),
				Error::<Runtime>::InsufficientLiquidity,
			);
		});
}
...
fn _swap_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(USDJ, DNAR), (50000, 10000));
			DexModule::_swap(USDJ, DNAR, 1000, 1000);
			assert_eq!(DexModule::get_liquidity(USDJ, DNAR), (51000, 9000));
			DexModule::_swap(DNAR, USDJ, 100, 800);
			assert_eq!(DexModule::get_liquidity(USDJ, DNAR), (50200, 9100));
		});
}
...
fn _swap_by_path_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(USDJ, DNAR), (50000, 10000));
			assert_eq!(DexModule::get_liquidity(USDJ, CHFJ), (100000, 10));
			DexModule::_swap_by_path(&vec![DNAR, USDJ], &vec![10000, 25000]);
			assert_eq!(DexModule::get_liquidity(USDJ, DNAR), (25000, 20000));
			DexModule::_swap_by_path(&vec![DNAR, USDJ, CHFJ], &vec![4000, 10000, 2]);
			assert_eq!(DexModule::get_liquidity(USDJ, DNAR), (15000, 24000));
			assert_eq!(DexModule::get_liquidity(USDJ, CHFJ), (110000, 8));
		});
}
...
fn add_liquidity_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::add_liquidity(Origin::signed(ALICE), USDJ, DNAR, 0, 100_000_000, 0, false),
				Error::<Runtime>::InvalidLiquidityIncrement
			);

			assert_eq!(DexModule::get_liquidity(USDJ, DNAR), (0, 0));
			assert_eq!(Tokens::free_balance(USDJ, &DexModule::account_id()), 0);
			assert_eq!(Tokens::free_balance(DNAR, &DexModule::account_id()), 0);
			assert_eq!(
				Tokens::free_balance(USDJ_DNAR_PAIR.get_dex_share_currency_id().unwrap(), &ALICE),
			...
			assert_eq!(
				DexModule::get_liquidity(USDJ, DNAR),
...
fn do_swap_with_exact_supply_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				DexModule::get_liquidity(USDJ, CHFJ),
			...
			assert_noop!(
				DexModule::do_swap_with_exact_supply(&BOB, &[DNAR, USDJ, CHFJ, EURJ], 100_000_000_000_000, 0, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			...
			assert_eq!(Tokens::free_balance(CHFJ, &DexModule::account_id()), 4_469_336_163);
			assert_eq!(Tokens::free_balance(USDJ, &BOB), 1_000_248_743_718_592_964);
...
fn do_swap_with_exact_target_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Tokens::free_balance(CHFJ, &DexModule::account_id()), 5_000_000_000);
			assert_eq!(Tokens::free_balance(USDJ, &BOB), 1_000_250_000_000_000_000);
			assert_eq!(Tokens::free_balance(DNAR, &BOB), 999_761_335_318_602_905);
			assert_eq!(Tokens::free_balance(CHFJ, &BOB), 1_000_000_005_000_000_000);
		});
}
...
fn initialize_added_liquidity_pools_genesis_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(USDJ, DNAR), (1000000, 2000000));
