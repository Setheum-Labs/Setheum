...
pub mod module {
	...
	pub enum Event<T: Config> {
		...
		/// supply_currency_amount, target_currency_amount\]
		BuyBackSwap(T::AccountId, Vec<CurrencyId>, Balance, Balance),
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn swap_with_exact_supply(
			...
			let _ = Self::do_swap_with_exact_supply(&who, &path, supply_amount, min_target_amount)?;
			Ok(().into())
		}
...
impl<T: Config> Pallet<T> {
	...
	fn _swap(
		...
		if let Some(trading_pair) = TradingPair::from_token_currency_ids(supply_currency_id, target_currency_id) {
			LiquidityPool::<T>::mutate(trading_pair, |(pool_0, pool_1)| -> DispatchResult {
					...
					if supply_currency_id == trading_pair.0 {
						...
						*pool_1 = pool_1.saturating_sub(target_decrement);
						if !supply_effect_amount.is_zero() {
							*pool_0 = pool_0.saturating_add(supply_effect_amount);
						};
					} else {
						*pool_0 = pool_0.saturating_sub(target_decrement);
						*pool_1 = pool_1.saturating_add(supply_increment);
						if !supply_effect_amount.is_zero() {
							*pool_1 = pool_1.saturating_add(supply_effect_amount);
						};
					};
					...
					if supply_currency_id == trading_pair.0 {
						...
						*pool_1 = pool_1.saturating_sub(target_decrement);
						if !target_effect_amount.is_zero() {
							*pool_1 = pool_1.saturating_add(target_effect_amount);
						};
					} else {
						*pool_0 = pool_0.saturating_sub(target_decrement);
						*pool_1 = pool_1.saturating_add(supply_increment);
						if !target_effect_amount.is_zero() {
							*pool_0 = pool_0.saturating_add(target_effect_amount);
						};
					};
	...
	/// Ensured atomic.
	#[transactional]
	fn buyback_do_efe_swap_with_exact_supply(
		who: &T::AccountId,
		path: &[CurrencyId],
		supply_amount: Balance,
		// min_target_amount: Balance,
		...
		if T::StableCurrencyIds::get().contains(&path[path.len() - 1]) {
			...
			let target_effect_amount = efe_actual_target_amount.checked_sub(actual_target_amount)
				.ok_or(Error::<T>::Overflow)?;
			// deposit the EFFECT to DEX and burn the boughtback amount.
			T::Currency::deposit(path[path.len() - 1], &module_account_id, target_effect_amount)?;
			T::Currency::transfer(path[0], &who, &module_account_id, supply_amount)?;
            Self::_efe_swap_by_path(&path, &efe_amounts, 0, target_effect_amount)?;
			T::Currency::withdraw(path[path.len() - 1], &module_account_id, efe_actual_target_amount)?;
	...
	/// Ensured atomic.
	#[transactional]
	fn buyback_do_efe_swap_with_exact_target(
		who: &T::AccountId,
		path: &[CurrencyId],
		target_amount: Balance,
		// max_supply_amount: Balance,
...
impl<T: Config> DEXManager<T::AccountId, CurrencyId, Balance> for Pallet<T> {
	...
	fn buyback_swap_with_exact_supply(
		...
		Self::buyback_do_efe_swap_with_exact_supply(who, path, supply_amount)
	}
	...
	fn buyback_swap_with_exact_target(
		...
		Self::buyback_do_efe_swap_with_exact_target(who, path, target_amount)
	}
