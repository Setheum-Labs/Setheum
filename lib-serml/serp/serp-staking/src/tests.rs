...
fn claim_reward_at_the_last_era_and_no_double_claim_and_invalid_claim() {
	...
	ExtBuilder::default().nominate(true).build_and_execute(|| {
		...
		assert_eq!(
			SerpPrices::get_relative_price(DNAR, DRAM),
			Some(Price::saturating_from_rational(1, 1)) // 1DNAR = 1DRAM
		);
		...
		let halved_total_payout_1 = total_payout_1 / 2;
		...
		let halved_total_payout_2 = total_payout_2 / 2;
...
fn test_max_nominator_rewarded_per_validator_and_cant_steal_someone_else_reward() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// TODO: Check if to Update to Tokens is needed!
		//
...
fn test_payout_stakers() {
	...
	ExtBuilder::default().has_stakers(false).build_and_execute(|| {
		...
		MockPriceSource::set_relative_price(Some(Price::one()));
		assert_eq!(
			PriceSource::get_relative_price(DNAR, DRAM),
			Some(Price::saturating_from_rational(1, 2)) // 1DNAR = 2DRAM
		);
