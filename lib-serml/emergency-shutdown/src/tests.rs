...
#[test]
fn open_reserve_refund_fail() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(EmergencyShutdownModule::can_refund(), false);
		assert_noop!(
			EmergencyShutdownModule::open_reserve_refund(Origin::signed(1)),
...
#[test]
fn open_reserve_refund_work() {
	ExtBuilder::default().build().execute_with(|| {
		System::set_block_number(1);
		assert_eq!(EmergencyShutdownModule::can_refund(), false);
		assert_ok!(EmergencyShutdownModule::emergency_shutdown(Origin::signed(1)));
		assert_noop!(
			EmergencyShutdownModule::open_reserve_refund(Origin::signed(5)),
			BadOrigin,
		);
		assert_ok!(EmergencyShutdownModule::open_reserve_refund(Origin::signed(1)));
...
#[test]
fn refund_reserves_fail() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			EmergencyShutdownModule::refund_reserves(Origin::signed(ALICE), 10),
