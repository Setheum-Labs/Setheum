...
fn len_of_deposit_of() {
	new_test_ext().execute_with(|| {
		for l in vec![0, 1, 200, 1000] {
			...
			DepositOf::<Runtime>::insert(2, value);
			assert_eq!(Democracy::len_of_deposit_of(2), Some(l));
		}

		DepositOf::<Runtime>::remove(2);
...
fn pre_image() {
	new_test_ext().execute_with(|| {
		...
		Preimages::<Runtime>::insert(key, missing);
		assert_noop!(Democracy::pre_image_data_len(key), Error::<Runtime>::PreimageMissing);
		assert_eq!(Democracy::check_pre_image_is_missing(key), Ok(()));

		Preimages::<Runtime>::remove(key);
		assert_noop!(Democracy::pre_image_data_len(key), Error::<Runtime>::PreimageMissing);
		assert_noop!(Democracy::check_pre_image_is_missing(key), Error::<Runtime>::NotImminent);
		...
		for l in vec![0, 10, 100, 1000u32] {
			...
			Preimages::<Runtime>::insert(key, available);
