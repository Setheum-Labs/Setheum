...
fn the_lock(amount: u64) -> BalanceLock<u64> {
	BalanceLock { id: DEMOCRACY_ID, amount }
}

/// TODO: Make up workind Tests for the commented tests
// #[test]
...
// 		for i in 1..=5 {
// 			assert_eq!(Tokens::locks(i, DRAM), vec![the_lock(i * 10)]);
// 		}
...
// 		assert_eq!(Tokens::locks(1, DRAM), vec![]);
// 		assert_eq!(Tokens::locks(2, DRAM), vec![the_lock(20)]);
// 		assert_eq!(Tokens::locks(3, DRAM), vec![the_lock(30)]);
// 		assert_eq!(Tokens::locks(4, DRAM), vec![the_lock(40)]);
// 		assert_eq!(Tokens::locks(5, DRAM), vec![]);
// 		assert_eq!(Tokens::free_balance(DRAM, &42), 2);
...
// 		assert_ok!(Democracy::unlock(Origin::signed(1), 4));
// 		assert_eq!(Tokens::locks(4, DRAM), vec![]);
...
// 		assert_ok!(Democracy::unlock(Origin::signed(1), 3));
// 		assert_eq!(Tokens::locks(3, DRAM), vec![]);
...
// 		fast_forward_to(18);
// 		assert_ok!(Democracy::unlock(Origin::signed(1), 2));
// 		assert_eq!(Tokens::locks(2, DRAM), vec![]);
// 	});
// }
...
fn no_locks_without_conviction_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Tokens::free_balance(DRAM, &42), 0);
...
// 		assert_eq!(Tokens::locks(5, DRAM), vec![the_lock(50)]);
// 		fast_forward_to(6);
// 		assert_ok!(Democracy::remove_other_vote(Origin::signed(1), 5, r.2));
// 		assert_ok!(Democracy::unlock(Origin::signed(5), 5));
// 		assert_eq!(Tokens::locks(5, DRAM), vec![the_lock(20)]);
// 		fast_forward_to(9);
// 		assert_noop!(
...
// 		assert_ok!(Democracy::unlock(Origin::signed(5), 5));
// 		assert_eq!(Tokens::locks(5, DRAM), vec![the_lock(10)]);
// 		fast_forward_to(17);
// 		assert_noop!(
...
// 		fast_forward_to(6);
// 		assert_ok!(Democracy::unlock(Origin::signed(5), 5));
// 		assert!(Tokens::locks(5, DRAM)[0].amount >= 20);

// 		fast_forward_to(10);
// 		assert_ok!(Democracy::unlock(Origin::signed(5), 5));
// 		assert!(Tokens::locks(5, DRAM)[0].amount >= 10);
...
// 		fast_forward_to(18);
// 		assert_ok!(Democracy::unlock(Origin::signed(5), 5));
// 		assert!(Tokens::locks(5, DRAM)[0].amount >= 5);
