...
use mock::{alice, bob, Event, SevmAccounts, ExtBuilder, Origin, Runtime, System, ALICE, BOB};
use std::str::FromStr;
...
fn claim_account_work() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(SevmAccounts::claim_account(
			...
			SevmAccounts::eth_sign(&alice(), &ALICE.encode(), &[][..])
		));
		...
		assert!(
			Accounts::<Runtime>::contains_key(SevmAccounts::eth_address(&alice()))
	...
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			SevmAccounts::claim_account(
				Origin::signed(ALICE),
				SevmAccounts::eth_address(&bob()),
				SevmAccounts::eth_sign(&bob(), &ALICE.encode(), &vec![1][..])
			),
		...
		assert_noop!(
			SevmAccounts::claim_account(
				Origin::signed(ALICE),
				SevmAccounts::eth_address(&bob()),
				SevmAccounts::eth_sign(&bob(), &BOB.encode(), &[][..])
			),
			Error::<Runtime>::InvalidSignature
		);
		assert_noop!(
		...
		assert_noop!(
			SevmAccounts::claim_account(
				Origin::signed(BOB),
				SevmAccounts::eth_address(&alice()),
				SevmAccounts::eth_sign(&alice(), &BOB.encode(), &[][..])
			),
...
fn evm_get_account_id() {
	ExtBuilder::default().build().execute_with(|| {
		let evm_account = SevmAccounts::eth_address(&alice());
...
fn account_to_evm() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let alice_evm_account = SevmAccounts::eth_address(&alice());
