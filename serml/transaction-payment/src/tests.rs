...
use mock::{
	AccountId, BlockWeights, Call, Currencies, Dex, ExtBuilder, Origin, Runtime, TransactionPayment, DNAR, ALICE,
	JUSD, BOB, DOT,
};
...
fn charges_fee_when_validate_and_native_is_not_enough() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Dex::get_liquidity_pool(DNAR, JUSD), (10000, 1000));
		...
		assert_eq!(Dex::get_liquidity_pool(DNAR, JUSD), (10000 - 2000, 1251));
	});
}
...
fn charge_fee_by_default_fee_token() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Dex::get_liquidity_pool(DNAR, JUSD), (10000, 1000));
		assert_eq!(Dex::get_liquidity_pool(DOT, JUSD), (100, 1000));
		assert_ok!(TransactionPayment::set_default_fee_token(
			Origin::signed(BOB),
			Some(DOT)
		));
		...
		assert_eq!(Dex::get_liquidity_pool(DNAR, JUSD), (10000 - 2000, 1251));
		assert_eq!(Dex::get_liquidity_pool(DOT, JUSD), (100 + 34, 1000 - 251));
	});
}
