...
use crate::precompile::{
	mock::{
		...
		Oracle, OraclePrecompile, Origin, Price, ScheduleCallPrecompile, System, Test, DNAR_ERC20_ADDRESS, ALICE, AUSD,
		XBTC,
	},
...
fn schedule_call_precompile_should_work() {
	new_test_ext().execute_with(|| {
		...
		// target
		U256::from(DNAR_ERC20_ADDRESS).to_big_endian(&mut input[3 * 32..4 * 32]);
		// value
