...
fn testnet_genesis(
	...
	use newrome_runtime::{
		...
		StakingPoolConfig, SudoConfig, SystemConfig, TechnicalCommitteeMembershipConfig,
		TokensConfig, VestingConfig, DNAR, JUSD,
	};
	#[cfg(feature = "std")]
	...
	newrome_runtime::GenesisConfig {
		...
		orml_tokens: TokensConfig {
			endowed_accounts: endowed_accounts
				.iter()
				.flat_map(|x| vec![(x.clone(), DNAR, initial_balance), (x.clone(), JUSD, initial_balance)])
				.collect(),
		},
...
fn newrome_genesis(
	...
	use newrome_runtime::{
		...
		OperatorMembershipBandConfig, OrmlNFTConfig, ParachainInfoConfig, StakingPoolConfig,
		SudoConfig, SystemConfig, TechnicalCommitteeMembershipConfig, TokensConfig, VestingConfig, DNAR, JUSD,
	};
	#[cfg(feature = "std")]
	use sp_std::collections::btree_map::BTreeMap;
	...
	newrome_runtime::GenesisConfig {
		...
		orml_tokens: TokensConfig {
			endowed_accounts: vec![
				(root_key.clone(), DNAR, initial_balance),
				(root_key, JUSD, initial_balance),
			],
		},
		module_airdrop: AirDropConfig {
			airdrop_accounts: {
